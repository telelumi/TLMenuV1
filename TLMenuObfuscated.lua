-- ════════════════════════════════════════════════════════════════════════════════
--  TLMENU UNIVERSAL COMPATIBILITY LAYER v3.0
--  Ensures script runs on all executors (Synapse, Fluxus, Krnl, Electron, etc.)
-- ════════════════════════════════════════════════════════════════════════════════
...
-- Drawing API Fallback (prevents crashes on mobile/low-end executors)
if not _genv.Drawing then
    local DrawingStub = {}
    function DrawingStub.new(objType)
        local base = {
            Visible = false, Transparency = 1, Color = Color3.new(1,1,1), Thickness = 1, ZIndex = 1,
...
-- FileSystem Fallback
if not _genv.writefile then _genv.writefile = function() end end
if not _genv.readfile then _genv.readfile = function() return nil end end
if not _genv.isfolder then _genv.isfolder = function() return false end end
if not _genv.makefolder then _genv.makefolder = function() end end
if not _genv.isfile then _genv.isfile = function() return false end end
if not _genv.appendfile then _genv.appendfile = function() end end
...
if not _genv.gethiddenproperty then _genv.gethiddenproperty = function(o, p) local s, r = pcall(function() return o[p] end) return s and r or nil end end
...
-- Math & CFrame Localizations (High Performance)
local _mfloor, _mceil, _mabs, _mclamp, _mmin, _mmax, _mrandom, _msqrt, _mrad, _mdeg, _mcos, _msin = math.floor, math.ceil, math.abs, math.clamp, math.min, math.max, math.random, math.sqrt, math.rad, math.deg, math.cos, math.sin
local _V3new, _V3_ZERO, _CFlookAt = Vector3.new, Vector3.new(0,0,0), CFrame.lookAt

-- TLCACHE System (Persistent Data)
local TLCACHE_DIR = "TLCACHE"
local _TLCACHE = {}
local function _initCache() if isfolder and not isfolder(TLCACHE_DIR) then pcall(function() makefolder(TLCACHE_DIR) end) end end
local function _saveCache(key, data) if not writefile then return end pcall(function() writefile(TLCACHE_DIR.."/"..key..".json", _SvcHttp:JSONEncode(data)) end) end
local function _loadCache(key) if not readfile or not isfile(TLCACHE_DIR.."/"..key..".json") then return nil end local ok, data = pcall(function() return _SvcHttp:JSONDecode(readfile(TLCACHE_DIR.."/"..key..".json")) end) return ok and data or nil end
_initCache()
...
-- -- Error Reporting (Master Debugger) ---------------------------
local function _handleError(err)
    ...
    local fullError = "╔══════════════════════════════════════════════╗\n" ..
                      ...
                      "╚══════════════════════════════════════════════╝"
    warn(fullError)
    print(fullError) -- Double print for executors with weird consoles
    pcall(function()
...
local _TL_state = { 
    ...
    movement = { tfActive = false, pfActive = false, antiRagdollEnabled = false },
    interaction = { _ctActive = false }
}
local _sc = {} -- Shared Scripts Table
local _u  = {} -- UI Tables
local _aim = {} -- Aimbot Module
...
_TL_state.currentMode, _TL_state.currentPage, _TL_state.totalPages, _TL_state.itemsPerPage = "emote", 1, 1, 8
_TL_state.isLoading = false
_TL_state.isLoadingEmotes, _TL_state.isLoadingAnimations = false, false
_TL_state.emotesData, _TL_state.originalEmotesData, _TL_state.filteredEmotes = {}, {}, {}
_TL_state.totalEmotesLoaded, _TL_state.favoriteEmotes = 0, {}
_TL_state.favoriteFileName, _TL_state.emoteSearchTerm = "FavoriteEmotes.json", ""
_TL_state.animationsData, _TL_state.originalAnimationsData, _TL_state.filteredAnimations = {}, {}, {}
_TL_state.favoriteAnimations         = {}
_TL_state.favoriteAnimationsFileName = "FavoriteAnimations.json"
_TL_state.animationSearchTerm        = ""
_TL_state.favoriteEnabled, _TL_state.favOnlyEnabled, _TL_state.emotesWalkEnabled = false, false, false
_TL_state.speedEmoteEnabled, _TL_state.currentEmoteTrack, _TL_state.isGUICreated, _TL_state.speedEmoteConfigFile = false, nil, false, "SpeedEmoteConfig.json"
-- UI Refs
_TL_state.Under, _TL_state.UIListLayout, _TL_state._1left, _TL_state._9right, _TL_state._4pages, _TL_state._3TextLabel, _TL_state._2Routenumber = nil, nil, nil, nil, nil, nil, nil
_TL_state.Top, _TL_state.EmoteWalkButton, _TL_state.Search, _TL_state.Favorite, _TL_state.FavOnlyBtn, _TL_state.SpeedEmote, _TL_state.SpeedBox, _TL_state.Changepage, _TL_state.Reload = nil, nil, nil, nil, nil, nil, nil, nil, nil
_TL_state.UICorner, _TL_state.UICorner1, _TL_state.UICorner2, _TL_state.UIListLayout_2, _TL_state.UICorner_2, _TL_state.UICorner_4, _TL_state.UICorner_5, _TL_state.UICorner_6 = nil, nil, nil, nil, nil, nil, nil, nil
_TL_state.defaultButtonImage = "rbxassetid://71408678974152"
_TL_state.enabledButtonImage, _TL_state.favoriteIconId, _TL_state.notFavoriteIconId = "rbxassetid://106798555684020", "rbxassetid://97307461910825", "rbxassetid://124025954365505"
_TL_state._clickConns, _TL_state._clickCooldown, _TL_state.COOLDOWN = {}, {}, 0.1
_TL_state._btnConns = {}
_TL_state._normalListCache = nil
_TL_state._emoteNameCache  = nil
_TL_state._pagesCache      = nil
_TL_state._frontButtonsCache = nil
_TL_state._clickConns, _TL_state._clickCooldown, _TL_state.COOLDOWN = {}, {}, 0.1
_TL_state._btnConns = {}

function _getNormalList()
    if _TL_state._normalListCache then return _TL_state._normalListCache end
    local src = _TL_state.currentMode == "animation" and _TL_state.filteredAnimations or _TL_state.filteredEmotes
    local t = {}
    for _, item in ipairs(src) do
        if not isInFavorites(item.id) then t[#t+1] = item end
    end
    _TL_state._normalListCache = t
    return t
end
...
for i = 1, #_TL_state._clickConns do
if _TL_state._clickConns[i] then _safeDisconnect(_TL_state._clickConns[i]) end
end
_TL_state._clickConns = {}
end
function _disconnectBtns()
for i = 1, #_TL_state._btnConns do
if _TL_state._btnConns[i] then _safeDisconnect(_TL_state._btnConns[i]) end
end
_TL_state._btnConns = {}
end
...
if writefile then writefile(_TL_state.favoriteFileName, AnimHttpService:JSONEncode(_TL_state.favoriteEmotes)) end
end
function saveFavoritesAnimations()
if writefile then writefile(_TL_state.favoriteAnimationsFileName, AnimHttpService:JSONEncode(_TL_state.favoriteAnimations)) end
end
function loadFavorites()
if readfile and isfile and isfile(_TL_state.favoriteFileName) then
local ok, r = pcall(function() return AnimHttpService:JSONDecode(readfile(_TL_state.favoriteFileName)) end)
if ok and r then
for _, f in ipairs(r) do f.id = tonumber(f.id) or f.id end
_TL_state.favoriteEmotes = r
end
end
end
function loadFavoritesAnimations()
if readfile and isfile and isfile(_TL_state.favoriteAnimationsFileName) then
local ok, r = pcall(function() return AnimHttpService:JSONDecode(readfile(_TL_state.favoriteAnimationsFileName)) end)
if ok and r then
for _, f in ipairs(r) do f.id = tonumber(f.id) or f.id end
_TL_state.favoriteAnimations = r
end
end
end
function loadSpeedEmoteConfig()
_TL_state.speedEmoteEnabled = false
_TL_state.emotesWalkEnabled = false
_TL_state.favoriteEnabled   = false
if readfile and isfile and isfile(_TL_state.speedEmoteConfigFile) then
local ok, r = pcall(function() return AnimHttpService:JSONDecode(readfile(_TL_state.speedEmoteConfigFile)) end)
...
_favSetEmote = {}
for _, f in ipairs(_TL_state.favoriteEmotes) do _favSetEmote[tostring(f.id)] = true end
_favSetAnim  = {}
for _, f in ipairs(_TL_state.favoriteAnimations) do _favSetAnim[tostring(f.id)] = true end
invalidatePagesCache()
end
function isInFavorites(assetId)
local key = tostring(assetId)
if _TL_state.currentMode == "animation" then return _favSetAnim[key] == true
else return _favSetEmote[key] == true end
end
function getActiveFavs()
if _TL_state.currentMode == "animation" then
if _TL_state.favOnlyEnabled then return _TL_state.favoriteAnimations end
return _G.filteredFavoritesAnimationsForDisplay or _TL_state.favoriteAnimations
else
if _TL_state.favOnlyEnabled then return _TL_state.favoriteEmotes end
return _G.filteredFavoritesForDisplay or _TL_state.favoriteEmotes
end
end
function calculateTotalPages()
if _pagesCache then return _pagesCache end
local favs = getActiveFavs()
if _TL_state.favOnlyEnabled then
_pagesCache = math.max(#favs > 0 and math.ceil(#favs / _TL_state.itemsPerPage) or 1, 1)
return _pagesCache
end
local favPg = #favs > 0 and math.ceil(#favs / _TL_state.itemsPerPage) or 0
local normal = _getNormalList()
local normPg = #normal > 0 and math.ceil(#normal / _TL_state.itemsPerPage) or 0
...
local si = (page-1)*_TL_state.itemsPerPage + 1
local ei = math.min(si + _TL_state.itemsPerPage - 1, #favs)
for i = si, ei do if favs[i] then items[#items+1] = favs[i] end end
return items
end
local favPgs = #favs > 0 and math.ceil(#favs / _TL_state.itemsPerPage) or 0
...
local si  = (adj-1)*_TL_state.itemsPerPage + 1
local ei  = math.min(si + _TL_state.itemsPerPage - 1, #normal)
...
if Reload then
local vis = (_TL_state.currentMode == "animation")
Reload.Visible = vis
...
icon.Image   = isFav and _TL_state.favoriteIconId or _TL_state.notFavoriteIconId
icon.Visible = true
end
...
local items   = getPageItems(_TL_state.currentPage)
_disconnectAll()
if _TL_state.currentMode == "animation" then
...
elseif _TL_state.favoriteEnabled then setFavoriteIcon(btn, false)
...
if _TL_state.favoriteEnabled then toggleFavoriteAnimation(capturedItem)
else applyAnimation(capturedItem) end
end)
_TL_state._clickConns[#_TL_state._clickConns+1] = conn
else
btn.Image = ""
...
elseif _TL_state.favoriteEnabled then setFavoriteIcon(capturedBtn, false) end
end
end)
end
end)
_TL_state._clickConns[#_TL_state._clickConns+1] = restoreConn
local cd = btn:FindFirstChild("ClickDetector")
if _TL_state.favoriteEnabled then
if not cd then
cd = Instance.new("TextButton"); cd.Name = "ClickDetector"
cd.Size = UDim2.new(1,0,1,0); cd.Position = UDim2.new(0,0,0,0)
cd.BackgroundTransparency = 1; cd.Text = ""
...
for _, e in ipairs(_TL_state.filteredEmotes) do if tonumber(e.id) == numId then name = e.name; break end end
for _, f in ipairs(_TL_state.favoriteEmotes) do if tonumber(f.id) == numId then name = f.name; break end end
end
...
applyHumanoidSlots()
if _TL_state.currentMode == "emote" then
...
if _4pages       then _4pages.Text       = tostring(_TL_state.totalPages)  end
if _2Routenumber then _2Routenumber.Text = tostring(_TL_state.currentPage) end
end
local _navCooldown, _emoteWalkBeforeFreeze = false, nil
local function goToPage(n)
_TL_state.currentPage = math.max(1, math.min(n, _TL_state.totalPages))
updatePageDisplay(); updateEmotes()
end
local function previousPage()
if _navCooldown then return end
_navCooldown = true
_TL_state.currentPage = _TL_state.currentPage <= 1 and _TL_state.totalPages or _TL_state.currentPage - 1
updatePageDisplay(); updateEmotes()
...
if _navCooldown then return end
_navCooldown = true
_TL_state.currentPage = _TL_state.currentPage >= _TL_state.totalPages and 1 or _TL_state.currentPage + 1
updatePageDisplay(); updateEmotes()
task.delay(0.12, function() _navCooldown = false end)
end
...
for i, f in ipairs(_TL_state.favoriteEmotes) do
if tostring(f.id) == key then found = true; idx = i; break end
end
if found then
table.remove(_TL_state.favoriteEmotes, idx); _favSetEmote[key] = nil
animNotif("Animations-Menu", '❌ "' .. cleanName .. '" removed', 3)
else
table.insert(_TL_state.favoriteEmotes, {id=emoteId, name=cleanName.." - ?"})
_favSetEmote[key] = true
animNotif("Animations-Menu", '★ "' .. cleanName .. '" added', 3)
end
saveFavorites(); invalidatePagesCache()
_TL_state.totalPages  = calculateTotalPages()
_TL_state.currentPage = math.min(_TL_state.currentPage, _TL_state.totalPages)
updatePageDisplay(); updateEmotes()
end
...
local found, idx = false, 0
for i, f in ipairs(_TL_state.favoriteAnimations) do
if tostring(f.id) == key then found = true; idx = i; break end
end
if found then
table.remove(_TL_state.favoriteAnimations, idx); _favSetAnim[key] = nil
animNotif("Animations-Menu", '❌ "' .. cleanName .. '" removed', 3)
else
table.insert(_TL_state.favoriteAnimations, {id=animData.id, name=cleanName.." - ?", bundledItems=animData.bundledItems})
...
_TL_state.favoriteEnabled = not _TL_state.favoriteEnabled
if Favorite then Favorite.Image = _TL_state.favoriteEnabled and _TL_state.favoriteIconId or _TL_state.notFavoriteIconId end
if _TL_state.favoriteEnabled then
...
_TL_state.favOnlyEnabled = not _TL_state.favOnlyEnabled
if FavOnlyBtn then
if _TL_state.favOnlyEnabled then
...
if _TL_state.currentMode == "emote" then _TL_state.emoteSearchTerm = ""; _TL_state.filteredEmotes = _TL_state.originalEmotesData
else _TL_state.animationSearchTerm = ""; _TL_state.filteredAnimations = _TL_state.originalAnimationsData end
else
...
invalidatePagesCache()
_TL_state.totalPages  = calculateTotalPages()
_TL_state.currentPage = 1
updatePageDisplay(); updateEmotes()
if _TL_state.favOnlyEnabled then
animNotif("Animations-Menu", "★ Nur Favorites – " .. _TL_state.totalPages .. " Seite(n)", 2)
else
...
track.Looped   = true
if _TL_state.speedEmoteEnabled or _TL_state.emotesWalkEnabled then
track:Play()
if _TL_state.speedEmoteEnabled then track:AdjustSpeed((SpeedBox and tonumber(SpeedBox.Text)) or 1) end
end
end
end
...
if _TL_state.isLoadingEmotes then return end
_TL_state.isLoadingEmotes = true; _TL_state.isLoading = true; _TL_state.emotesData = {}; _TL_state.totalEmotesLoaded = 0
...
if d.id and d.id > 0 then _TL_state.emotesData[#_TL_state.emotesData+1] = d; _TL_state.totalEmotesLoaded = _TL_state.totalEmotesLoaded + 1 end
end
else
_TL_state.emotesData = {{id=3360686498,name="Stadium"},{id=3360692915,name="Tilt"},{id=3576968026,name="Shrug"},{id=3360689775,name="Salute"}}
_TL_state.totalEmotesLoaded = #_TL_state.emotesData
end
_TL_state.originalEmotesData = _TL_state.emotesData; _TL_state.filteredEmotes = _TL_state.emotesData
_emoteNameCache = {}
for _, e in ipairs(_TL_state.emotesData) do _emoteNameCache[tostring(e.id)] = e.name end
invalidatePagesCache()
_TL_state.totalPages = calculateTotalPages(); _TL_state.currentPage = 1; updatePageDisplay(); updateEmotes()
animNotif("Animations-Menu","📦 ".._TL_state.totalEmotesLoaded.." Emotes loaded!", 4)
_TL_state.isLoadingEmotes = false; _TL_state.isLoading = false
end
local function fetchAllAnimations()
if _TL_state.isLoadingAnimations then return end
_TL_state.isLoadingAnimations = true; _TL_state.animationsData = {}
local ok, result = pcall(function()
...
if d.id and d.id > 0 then _TL_state.animationsData[#_TL_state.animationsData+1] = d end
end
end
_TL_state.originalAnimationsData = _TL_state.animationsData; _TL_state.filteredAnimations = _TL_state.animationsData
_TL_state.isLoadingAnimations = false
end
local function searchEmotes(term)
if _TL_state.isLoading then animNotif("Animations-Menu","⏳ Loading...",2); return end
term = term:lower()
if term == "" then
_TL_state.filteredEmotes = _TL_state.originalEmotesData
_G.filteredFavoritesForDisplay = nil
else
local isId = term:match("^%d%d%d%d%d+$")
local newList = {}
if isId then
for _, e in ipairs(_TL_state.originalEmotesData) do
...
_TL_state.originalEmotesData[#_TL_state.originalEmotesData+1] = ne
...
_TL_state.filteredEmotes = newList
if not isId then
_G.filteredFavoritesForDisplay = {}
for _, f in ipairs(_TL_state.favoriteEmotes) do
...
term = term:lower()
if term == "" then
_TL_state.filteredAnimations = _TL_state.originalAnimationsData
_G.filteredFavoritesAnimationsForDisplay = nil
else
local isId = term:match("^%d+$")
local newList = {}
if isId then
for _, a in ipairs(_TL_state.originalAnimationsData) do
...
_TL_state.filteredAnimations = newList
if not isId then
_G.filteredFavoritesAnimationsForDisplay = {}
for _, f in ipairs(_TL_state.favoriteAnimations) do
...
_TL_state.emotesWalkEnabled = not _TL_state.emotesWalkEnabled
if EmoteWalkButton then EmoteWalkButton.Image = _TL_state.emotesWalkEnabled and _TL_state.enabledButtonImage or _TL_state.defaultButtonImage end
task.defer(stopCurrentEmote)
end
if getgenv then
_genv.TLAnimFreeze = function(on)
if on then
_emoteWalkBeforeFreeze = _TL_state.emotesWalkEnabled
_TL_state.emotesWalkEnabled = true
if EmoteWalkButton then EmoteWalkButton.Image = _TL_state.enabledButtonImage end
else
_emoteWalkBeforeFreeze = nil
_TL_state.emotesWalkEnabled = false
if EmoteWalkButton then EmoteWalkButton.Image = _TL_state.defaultButtonImage end
pcall(stopCurrentEmote); pcall(stopAnimEmotes)
end
end
end
local function toggleSpeedEmote()
_TL_state.speedEmoteEnabled = not _TL_state.speedEmoteEnabled
if SpeedBox then SpeedBox.Visible = _TL_state.speedEmoteEnabled end
animNotif("Animations-Menu", _TL_state.speedEmoteEnabled and "⚡ Speed Emote ON" or "⚡ Speed Emote OFF", 2)
task.defer(stopCurrentEmote)
if writefile then
writefile(_TL_state.speedEmoteConfigFile, AnimHttpService:JSONEncode({
Enabled=_TL_state.speedEmoteEnabled,
...
_disconnectAll()
if _TL_state.favOnlyEnabled then
_TL_state.favOnlyEnabled = false
if FavOnlyBtn then
FavOnlyBtn.BackgroundColor3       = Color3.fromRGB(255,255,255)
FavOnlyBtn.BackgroundTransparency = 0.92
FavOnlyBtn.TextColor3             = Color3.fromRGB(210,210,210)
...
_G.filteredFavoritesAnimationsForDisplay = nil
if _TL_state.currentMode == "emote" then
_TL_state.currentMode = "animation"
animNotif("Animations-Menu","🎬 Mode: Animation",2)
task.spawn(function()
fetchAllAnimations()
if Search then Search.Text = _TL_state.animationSearchTerm end
invalidatePagesCache()
_TL_state.totalPages = calculateTotalPages(); _TL_state.currentPage = 1
updatePageDisplay(); updateEmotes()
end)
else
_TL_state.currentMode = "emote"
animNotif("Animations-Menu","🎬 Mode: Emote",2)
if Search then Search.Text = _TL_state.emoteSearchTerm end
invalidateFrontButtonsCache()
invalidatePagesCache()
_TL_state.totalPages = calculateTotalPages(); _TL_state.currentPage = 1
updatePageDisplay(); updateEmotes()
...
if Reload then
local isAnim = (_TL_state.currentMode == "animation")
...
if not (_TL_state.emotesWalkEnabled or _TL_state.speedEmoteEnabled) then return end
...
_TL_state.emotesWalkEnabled = false; _TL_state.speedEmoteEnabled = false
_TL_state.favoriteEnabled   = false; _TL_state.favOnlyEnabled    = false
currentEmoteTrack = nil
stopAnimEmotes(); stopCurrentEmote()
pcall(function()
if EmoteWalkButton then EmoteWalkButton.Image = _TL_state.defaultButtonImage end
if Favorite        then Favorite.Image        = _TL_state.notFavoriteIconId   end
...
EmoteWalkButton = makeIconBtn(Top,"EmoteWalkButton",_TL_state.defaultButtonImage,btnStartX,          "Walk",  true)
Favorite        = makeIconBtn(Top,"Favorite",       _TL_state.notFavoriteIconId, btnStartX+btnGap,   "Favs",  true)
FavOnlyBtn      = makeIconBtn(Top,"FavOnlyBtn",     "★",              btnStartX+btnGap*2, "Only",  false)
SpeedEmote      = makeIconBtn(Top,"SpeedEmote",     _TL_state.defaultButtonImage,btnStartX+btnGap*3, "Speed", true)
...
_TL_state.emotesWalkEnabled=false; _TL_state.speedEmoteEnabled=false; _TL_state.favoriteEnabled=false; _TL_state.favOnlyEnabled=false
EmoteWalkButton.Image=_TL_state.defaultButtonImage; Favorite.Image=_TL_state.notFavoriteIconId
if FavOnlyBtn then
FavOnlyBtn.BackgroundColor3=Color3.fromRGB(255,255,255)
FavOnlyBtn.BackgroundTransparency=0.92
FavOnlyBtn.TextColor3=Color3.fromRGB(210,210,210)
...
local now = os.clock()
if not _TL_state._clickCooldown[name] or (now - _TL_state._clickCooldown[name]) > _TL_state.COOLDOWN then
_TL_state._clickCooldown[name] = now; cb()
end
end
connectEvents = function()
_disconnectBtns()
if _1left  then _TL_state._btnConns[#_TL_state._btnConns+1] = _1left.MouseButton1Click:Connect(previousPage) end
if _9right then _TL_state._btnConns[#_TL_state._btnConns+1] = _9right.MouseButton1Click:Connect(nextPage)     end
if _2Routenumber then
_TL_state._btnConns[#_TL_state._btnConns+1] = _2Routenumber.FocusLost:Connect(function()
local p = tonumber(_2Routenumber.Text)
if p then goToPage(p) else _2Routenumber.Text = tostring(_TL_state.currentPage) end
end)
end
if Search then
local _searchDebounceThread = nil
_TL_state._btnConns[#_TL_state._btnConns+1] = Search.Changed:Connect(function(prop)
...
if _TL_state.currentMode == "emote" then
_TL_state.emoteSearchTerm = capturedText; searchEmotes(_TL_state.emoteSearchTerm)
else
_TL_state.animationSearchTerm = capturedText; searchAnimations(_TL_state.animationSearchTerm)
end
end)
end)
end
if EmoteWalkButton then _TL_state._btnConns[#_TL_state._btnConns+1] = EmoteWalkButton.MouseButton1Click:Connect(function() safe("Walk",  toggleEmoteWalk)    end) end
if Favorite        then _TL_state._btnConns[#_TL_state._btnConns+1] = Favorite.MouseButton1Click:Connect(       function() safe("Favs",  toggleFavoriteMode) end) end
if FavOnlyBtn      then _TL_state._btnConns[#_TL_state._btnConns+1] = FavOnlyBtn.MouseButton1Click:Connect(     function() safe("FavOnly",toggleFavOnly)     end) end
if SpeedEmote      then _TL_state._btnConns[#_TL_state._btnConns+1] = SpeedEmote.MouseButton1Click:Connect(     function() safe("Speed", toggleSpeedEmote)   end) end
if AvatarOutfitBtn then _TL_state._btnConns[#_TL_state._btnConns+1] = AvatarOutfitBtn.MouseButton1Click:Connect(function()
...
if Reload          then _TL_state._btnConns[#_TL_state._btnConns+1] = Reload.MouseButton1Click:Connect(         function() safe("Reload",toggleAutoReload)   end) end
if Changepage      then _TL_state._btnConns[#_TL_state._btnConns+1] = Changepage.MouseButton1Click:Connect(     function() safe("Mode",  switchMode)         end) end
if SpeedBox then
_TL_state._btnConns[#_TL_state._btnConns+1] = SpeedBox.FocusLost:Connect(function()
if writefile then
writefile(_TL_state.speedEmoteConfigFile, AnimHttpService:JSONEncode({
Enabled=_TL_state.speedEmoteEnabled, SpeedValue=tonumber(SpeedBox.Text) or 1
}))
end
end)
end
...
if #_TL_state.emotesData > 0 then updatePageDisplay(); updateEmotes(); loadSpeedEmoteConfig() end
end
end)
end))
...
if _TL_state.speedEmoteEnabled and not _TL_state.emotesWalkEnabled then
...
if not _tlAlive() then return end
if #_TL_state.originalEmotesData == 0 and not _TL_state.isLoadingEmotes then
fetchAllEmotes()
end
end)
...
if locale:sub(1,2) == "de" then
    ...
    T.settings_general = "Allgemein"
    T.settings_keybinds = "Tastenbelegung"
    T.settings_sound = "Soundeffekte"
    T.save_settings = "💾 Einstellungen & Tasten speichern"
    T.qa_title = "SCHNELLAKTIONEN"
    T.qa_subtitle = "Aktion wählen"
    T.saved_outfits_title = "Gespeicherte Outfits"
    T.saved_outfits_sub = "Deine Outfits verwalten"
    T.new_folder = "Neuer Ordner"
    T.folder_prefix = "Ordner: "
    T.unknown = "Unbekannt"
    T.no_outfits_saved = "— Keine Outfits gespeichert"
else
...
local _invisAnimEnabled = true  -- ghost sink/rise animation toggle
...
h.Adornee             = ch
h.FillColor           = Color3.fromRGB(220, 235, 255) -- faint glass/camo tint
h.OutlineColor        = Color3.fromRGB(255, 255, 255)
h.FillTransparency    = 0.85 -- almost invisible but recognizable
h.OutlineTransparency = 1.0  -- Outline entfernt (wie gewünscht)
...
local function _startInvisHeartbeat()
...
local function setInvis(on)
    ...
    -- Cancel any in-progress ghost animation
    ...
    if not on then
        -- Ghost RISE: animate from underground to savedCF over 1.5s
        local savedCF = _invisSavedCF
        _invisSavedCF = nil
        _G._TLInvisSavedCF = nil
        if hum then
            hum.Health = hum.MaxHealth
            pcall(function() hum.CameraOffset = Vector3.zero end)
        end
        if root and savedCF then
            local tPos = savedCF.Position
            local startY = tPos.Y - 5
            local savedRot = savedCF - savedCF.Position  -- rotation-only CFrame
            pcall(function() root.CFrame = CFrame.new(tPos.X, startY, tPos.Z) * savedRot end)
            root.AssemblyLinearVelocity = Vector3.zero
            local elapsed = 0
            local _riseTrack = nil
            -- Play rise emote via reliable QA loader (replicates to other players)
            if _invisAnimEnabled then
            task.spawn(function()
                local track = _AF_getReliableActionTrack(hum, "113326574345610", "GhostRise")
                if track then track.Looped = false; _riseTrack = track end
            end)
            end
            _invGhostConn = _SvcRS.Heartbeat:Connect(function(dt)
                elapsed = elapsed + dt
                local t = math.min(elapsed / 1.5, 1)
                local ease = 1 - (1 - t) * (1 - t)  -- ease out
                pcall(function()
                    root.CFrame = CFrame.new(tPos.X, startY + (tPos.Y - startY) * ease, tPos.Z) * savedRot
                end)
                for _, entry in ipairs(invisParts) do
                    local p = entry.part
                    if p and p.Parent then
                        pcall(function() p.Transparency = math.max(entry.origTransp, 0.99 * (1 - ease)) end)
                    end
                end
                if t >= 1 then
                    _invGhostConn:Disconnect(); _invGhostConn = nil
                    if _riseTrack then pcall(function() _riseTrack:Stop() end); _riseTrack = nil end
                    for _, entry in ipairs(invisParts) do
                        local p = entry.part
                        if p and p.Parent then pcall(function() p.Transparency = entry.origTransp end) end
                    end
                    invisParts = {}
    ...
    -- Ghost SINK: fade out + sink over 1.5s, then start full invis heartbeat
    local initCF = root and root.CFrame
    if initCF then
        _invisSavedCF = initCF
        _G._TLInvisSavedCF = initCF
    end
    local startY = initCF and initCF.Position.Y or 0
    local elapsed = 0
    local _sinkTrack = nil
    -- Play sink emote via reliable QA loader (replicates to other players)
    if _invisAnimEnabled then
    task.spawn(function()
        local track = _AF_getReliableActionTrack(hum, "139880073845125", "GhostSink")
        if track then track.Looped = false; _sinkTrack = track end
    end)
    end
    _invGhostConn = _SvcRS.Heartbeat:Connect(function(dt)
        if not invisActive then _invGhostConn:Disconnect(); _invGhostConn = nil; return end
        elapsed = elapsed + dt
        local t = math.min(elapsed / 1.5, 1)
        local ease = t * t  -- ease in (accelerates downward)
        local newY = startY - 5 * ease
        pcall(function()
            root.CFrame = CFrame.new(initCF.Position.X, newY, initCF.Position.Z)
        end)
        for _, entry in ipairs(invisParts) do
            local p = entry.part
            if p and p.Parent then
                pcall(function() p.Transparency = math.min(0.99, entry.origTransp + (0.99 - entry.origTransp) * ease) end)
            end
        end
        if t >= 1 then
            _invGhostConn:Disconnect(); _invGhostConn = nil
            if _sinkTrack then pcall(function() _sinkTrack:Stop() end); _sinkTrack = nil end
            _startInvisHeartbeat()
        end
    end)
end
...
-- ╔═══════════════════════════════════════════════════╗
-- ║  TL FLY (TL FLY SYSTEM HUD + Main Panel)          ║
-- ╚═══════════════════════════════════════════════════╝
...
local RUNTIME_KEY_FLY = "__TL_Fly_Runtime"
...
do -- Inner scope for TL Fly
    ...
    -- -- Helpers -----------------------------------------------------------------
    local _fly_conns = {}
    local function bindFly(event, fn) local c = event:Connect(fn); table.insert(_fly_conns, c); return c end
    local function corner(obj, r) local c = Instance.new("UICorner", obj); c.CornerRadius = UDim.new(0, r); return c end
    ...
    local speedLevels = {
        ...
        { name="IDLE",   speed=110, accel=4.5,  decel=2.2, color=Color3.fromRGB(120,200,255), bar=0.5 },
    ...
    -- -- Workspace Cache (persists selected anim set across re-injections) --------
    ...
    if not _flyCache then
        _flyCache = Instance.new("Folder")
        _flyCache.Name = "_TL_FlyCache"
        _flyCache.Parent = workspace
        _wsWasFresh = true
    end
    local _cacheAnimIdx = _flyCache:FindFirstChild("AnimSetIndex")
    if not _cacheAnimIdx then
        _cacheAnimIdx = Instance.new("NumberValue")
        _cacheAnimIdx.Name = "AnimSetIndex"
        _cacheAnimIdx.Value = 1
        _cacheAnimIdx.Parent = _flyCache
        _wsWasFresh = true
    end
    -- If workspace cache is fresh (server change / first load), restore from file
    if _wsWasFresh then
        local saved = _loadFlyAnimSet()
        if saved then
            _cacheAnimIdx.Value = saved
        end
    end
    ...
    Wrapper.Size                   = UDim2.new(0, 720, 0, 34)
    Wrapper.AnchorPoint            = Vector2.new(0.5, 0)
    Wrapper.Position               = UDim2.new(0.5, 0, 0, -38) -- Moved 5px down
    ...
    mStroke.Thickness    = 1; mStroke.Color = C.bg3; mStroke.Transparency = 0; mStroke.Parent = MainFrame
    ...
    Title.Size = UDim2.new(1, 0, 0, 14); Title.Position = UDim2.new(0, 0, 0, 5); Title.BackgroundTransparency = 1; Title.Text = "TL FLY"; Title.TextColor3 = C.accent
    ...
    StatusLabel.Size = UDim2.new(1, 0, 0, 12); StatusLabel.Position = UDim2.new(0, 0, 0, 19); StatusLabel.BackgroundTransparency = 1; StatusLabel.Text = "GLIDE"; StatusLabel.TextColor3 = C.sub
    StatusLabel.Font = Enum.Font.GothamBold; StatusLabel.TextSize = 9; StatusLabel.TextXAlignment = Enum.TextXAlignment.Left; StatusLabel.Parent = InfoCont

    local function createButton(text, callback)
        local btn = Instance.new("TextButton")
        btn.Size = UDim2.new(0, 100, 0, 30); btn.BackgroundColor3 = C.bg3; btn.BackgroundTransparency = 0.2
        btn.TextColor3 = C.text; btn.Font = Enum.Font.GothamBold; btn.TextSize = 10; btn.Text = text; btn.BorderSizePixel = 0; btn.AutoButtonColor = false
        corner(btn, 8)
        local s = stroke(btn, 1, C.bg3, 0)
        local popS = Instance.new("UIScale"); popS.Parent = btn
        bindFly(btn.MouseEnter, function() TweenService:Create(btn, TweenInfo.new(0.1), {BackgroundColor3 = C.bg3, BackgroundTransparency = 0.05, TextColor3 = C.accent}):Play() end)
        bindFly(btn.MouseLeave, function() TweenService:Create(btn, TweenInfo.new(0.15), {BackgroundColor3 = C.bg3, BackgroundTransparency = 0.2, TextColor3 = C.text}):Play() end)
    ...
    local MinusBtn = createButton("-", function() if speedIndex > 1 then speedIndex = speedIndex - 1; hasBoosted = true; updateFlyPanel() end end)
    MinusBtn.Size = UDim2.new(0, 26, 1, 0); MinusBtn.LayoutOrder = 2; MinusBtn.Parent = SpeedGroup
    local PlusBtn = createButton("+", function() if speedIndex < #speedLevels then speedIndex = speedIndex + 1; hasBoosted = true; updateFlyPanel() end end)
    ...
    -- Speed HUD Replacement (Slider style)
    local SpeedHUDReplace = Instance.new("Frame")
    SpeedHUDReplace.Size = UDim2.new(0, 110, 1, 0); SpeedHUDReplace.BackgroundTransparency = 1; SpeedHUDReplace.LayoutOrder = 5; SpeedHUDReplace.Parent = ButtonContainer
    
    local SpeedLiveLabel = Instance.new("TextLabel")
    SpeedLiveLabel.Size = UDim2.new(1, 0, 0, 14); SpeedLiveLabel.Position = UDim2.new(0, 0, 0, 5); SpeedLiveLabel.BackgroundTransparency = 1
    SpeedLiveLabel.Text = "SPEED: 0"; SpeedLiveLabel.TextColor3 = Color3.new(1, 1, 1); SpeedLiveLabel.Font = Enum.Font.GothamBold; SpeedLiveLabel.TextSize = 10
    SpeedLiveLabel.TextXAlignment = Enum.TextXAlignment.Left; SpeedLiveLabel.Parent = SpeedHUDReplace
    
    local SliderBg = Instance.new("Frame")
    SliderBg.Size = UDim2.new(1, 0, 0, 4); SliderBg.Position = UDim2.new(0, 0, 0, 20); SliderBg.BackgroundColor3 = Color3.fromRGB(40, 40, 45); SliderBg.BorderSizePixel = 0; SliderBg.Parent = SpeedHUDReplace; corner(SliderBg, 4)
    ...
    -- Dropdown Pill --
    local PILL_EXPANDED_H, PILL_GAP = 188, 6
    local dropOpen = false
    local PillOuter = Instance.new("Frame")
    PillOuter.Size = UDim2.new(0, 160, 0, 0); PillOuter.Position = UDim2.new(0, 464, 0, 34 + PILL_GAP)
    PillOuter.BackgroundColor3 = C.bg2; PillOuter.BackgroundTransparency = 0.06
    PillOuter.BorderSizePixel = 0; PillOuter.ClipsDescendants = true; PillOuter.Parent = Wrapper
    corner(PillOuter, 14)
    local pillStroke = _makeDummyStroke(); pillStroke.Color = C.bg3; pillStroke.Thickness = 1.5; pillStroke.Transparency = 0.22; pillStroke.Parent = PillOuter

    local ScrollFrame = Instance.new("ScrollingFrame")
    ScrollFrame.Size = UDim2.new(1, 0, 1, 0); ScrollFrame.Position = UDim2.new(0, 0, 0, 0); ScrollFrame.BackgroundTransparency = 1; ScrollFrame.BorderSizePixel = 0; ScrollFrame.ScrollBarThickness = 3; ScrollFrame.ScrollBarImageColor3 = C.gradL or C.accent; ScrollFrame.ScrollBarImageTransparency = 0.4; ScrollFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y; ScrollFrame.Parent = PillOuter
    local scrollLayout = Instance.new("UIListLayout"); scrollLayout.Padding = UDim.new(0, 2); scrollLayout.Parent = ScrollFrame
    local scrollPadding = Instance.new("UIPadding"); scrollPadding.PaddingTop = UDim.new(0, 4); scrollPadding.PaddingBottom = UDim.new(0, 4); scrollPadding.PaddingLeft = UDim.new(0, 4); scrollPadding.Parent = ScrollFrame

    local function refreshAnimRows()
        for i, btn in ipairs(animSetButtons) do
            local isActive = (i == currentAnimSet)
            TweenService:Create(btn, TweenInfo.new(0.08), {BackgroundTransparency = isActive and 0.55 or 0.85}):Play()
    ...
    local function switchAnimSet(index)
        ...
        -- Persist to workspace cache AND file (survives server changes)
    ...
    for i, set in ipairs(animSets) do
        ...
        row.Size = UDim2.new(1, -8, 0, 34); row.BackgroundColor3 = C.bg3; row.BackgroundTransparency = 0.85; row.BorderSizePixel = 0; row.AutoButtonColor = false; row.Text = ""; row.Parent = ScrollFrame
        corner(row, 10)
        ...
        local dot = Instance.new("Frame"); dot.Name = "ActiveDot"; dot.Size = UDim2.new(0, 5, 0, 5); dot.Position = UDim2.new(1, -12, 0.5, -2); dot.BackgroundColor3 = C.accent; dot.BorderSizePixel = 0; dot.Visible = (i == 1); dot.Parent = row
        corner(dot, 99)
        
        bindFly(row.MouseEnter, function() TweenService:Create(row, TweenInfo.new(0.1), {BackgroundTransparency=0.15}):Play(); if i ~= currentAnimSet then TweenService:Create(nameLbl, TweenInfo.new(0.1), {TextColor3=C.accent}):Play() end end)
        bindFly(row.MouseLeave, function() local isActive = (i == currentAnimSet); TweenService:Create(row, TweenInfo.new(0.1), {BackgroundTransparency=isActive and 0.55 or 0.85}):Play(); if not isActive then TweenService:Create(nameLbl, TweenInfo.new(0.1), {TextColor3=C.text}):Play() end end)
    ...
    updateFlyPanel = function()
        ...
        if SpeedBtn then SpeedBtn.Text = "Speed: " .. data.name end
        ...
        if SliderFill then TweenService:Create(SliderFill, TweenInfo.new(0.15), {Size = UDim2.new(data.bar, 0, 1, 0), BackgroundColor3 = data.color}):Play() end
        if speedIndex == 4 and flying then local flash = registerFlyInstance(Instance.new("Frame", ScreenGui)); flash.Size = UDim2.new(1,0,1,0); flash.BackgroundColor3 = Color3.new(1,1,1); flash.BackgroundTransparency = 0.9; TweenService:Create(flash, TweenInfo.new(0.2), {BackgroundTransparency=1}):Play(); task.delay(0.2, function() flash:Destroy() end) end
    end
    ...
    local function startFly()
        ...
        while flying do
            ...
            elseif ctrl.f == 1 and speedIndex >= 3 then
                if speedIndex == 4 and flyFwd2Track then
            ...
            if lastPlayedTrack ~= targetTrack then
                if lastPlayedTrack then lastPlayedTrack:Stop(0.35) end
                if targetTrack then targetTrack:Play(0.35) end
            ...
            if SpeedLiveLabel then SpeedLiveLabel.Text = "SPEED: " .. math.floor(currentVel.Magnitude + 0.5) end
            
            -- Floor clipping prevention
            local rp = RaycastParams.new()
            rp.FilterDescendantsInstances = {myChar}
            rp.FilterType = Enum.RaycastFilterType.Exclude
            local ray = workspace:Raycast(myHRP.Position, Vector3.new(0, -3.2, 0), rp)
            if ray then
                local hoverY = ray.Position.Y + 3.0
                if myHRP.Position.Y < hoverY then
                    currentVel = Vector3.new(currentVel.X, math.max(currentVel.Y, (hoverY - myHRP.Position.Y) * 8), currentVel.Z)
                end
            end
        ...
        if flyFwd2Track then flyFwd2Track:Stop(0.35) end
        lastPlayedTrack = nil
...
_u.secGame.Size = UDim2.new(1, -PAD*2, 0, 13)
...
_u.secGame.TextColor3 = _C3_WHITE
...
_u.gameCard.BackgroundColor3 = C.bg2 or _C3_BG2
_u.gameCard.BorderSizePixel = 0
corner(_u.gameCard, 12)
_u.gameCardS = _makeDummyStroke(_u.gameCard)
_u.gameCardS.Thickness = 1; _u.gameCardS.Color = C.bg3 or _C3_BG3; _u.gameCardS.Transparency = 0.28
local iconSz = 76
_u.iconWrap = Instance.new("Frame", _u.gameCard)
_u.iconWrap.Size = UDim2.new(0, iconSz, 0, iconSz)
_u.iconWrap.Position = UDim2.new(0, 12, 0.5, -iconSz/2)
_u.iconWrap.BackgroundColor3 = Color3.fromRGB(2, 12, 5)
_u.iconWrap.BorderSizePixel = 0
corner(_u.iconWrap, 10)
_u.iconWrapS = _makeDummyStroke(_u.iconWrap)
_u.iconWrapS.Thickness = 1; _u.iconWrapS.Color = C.accent or C.bg3; _u.iconWrapS.Transparency = 0.5
_u.gameIcon = Instance.new("ImageLabel", _u.iconWrap)
_u.gameIcon.Size = UDim2.new(1, -4, 1, -4)
_u.gameIcon.Position = UDim2.new(0, 2, 0, 2)
_u.gameIcon.BackgroundTransparency = 1
_u.gameIcon.Image = _gameThumb
_u.gameIcon.ScaleType = Enum.ScaleType.Crop
corner(_u.gameIcon, 8)
local infoX = 12 + iconSz + 10
_u.gameTitleLbl = Instance.new("TextLabel", _u.gameCard)
_u.gameTitleLbl.Size = UDim2.new(1, -infoX - 10, 0, 22)
_u.gameTitleLbl.Position = UDim2.new(0, infoX, 0, 10)
_u.gameTitleLbl.BackgroundTransparency = 1
_u.gameTitleLbl.Text = _gameTitle
_u.gameTitleLbl.Font = Enum.Font.GothamBlack; _u.gameTitleLbl.TextSize = 14
...
_u.linePlace.Size = UDim2.new(1, -infoX - 10, 0, 16)
_u.linePlace.Position = UDim2.new(0, infoX, 0, 36)
_u.linePlace.BackgroundTransparency = 1
_u.linePlace.Text = T.home_place_id .. ":  " .. tostring(_placeId)
_u.linePlace.Font = Enum.Font.GothamBold; _u.linePlace.TextSize = 11
...
if _universeId > 0 then
    ...
    _u.lineUni.Size = UDim2.new(1, -infoX - 10, 0, 16)
    ...
    _u.lineUni.Font = Enum.Font.GothamBold; _u.lineUni.TextSize = 11
...
_u.secProf.Size = UDim2.new(1, -PAD*2, 0, 13)
...
_u.secProf.TextColor3 = _C3_WHITE
...
_u.profCard.BackgroundColor3 = C.bg2 or _C3_BG2
_u.profCard.BorderSizePixel = 0
corner(_u.profCard, 12)
_u.profCardS = _makeDummyStroke(_u.profCard)
_u.profCardS.Thickness = 1; _u.profCardS.Color = C.bg3 or _C3_BG3; _u.profCardS.Transparency = 0.28
local profAvSize = 56
_u.profAvWrap = Instance.new("Frame", _u.profCard)
_u.profAvWrap.Size = UDim2.new(0, profAvSize, 0, profAvSize)
_u.profAvWrap.Position = UDim2.new(0, 12, 0.5, -profAvSize/2)
_u.profAvWrap.BackgroundColor3 = Color3.fromRGB(2, 14, 6)
_u.profAvWrap.BorderSizePixel = 0
corner(_u.profAvWrap, 99)
_u.profAvWrapS = _makeDummyStroke(_u.profAvWrap)
_u.profAvWrapS.Thickness = 1.5; _u.profAvWrapS.Color = C.accent; _u.profAvWrapS.Transparency = 0.38
_u.avClip = Instance.new("Frame", _u.profAvWrap)
_u.avClip.Size = UDim2.new(1,-4,1,-4); _u.avClip.Position = UDim2.new(0,2,0,2)
_u.avClip.BackgroundTransparency = 1; _u.avClip.BorderSizePixel = 0
_u.avClip.ClipsDescendants = true; corner(_u.avClip, 99)
_u.homeAvatar = Instance.new("ImageLabel", _u.avClip)
...
_u.nameLbl.Font = Enum.Font.GothamBlack; _u.nameLbl.TextSize = 16
_u.nameLbl.TextColor3 = C.text or Color3.fromRGB(230,232,245)
...
_u.tagLbl.Font = Enum.Font.GothamBold; _u.tagLbl.TextSize = 11
...
_u.ageLbl = Instance.new("TextLabel", _u.profCard)
_u.ageLbl.Size = UDim2.new(1, -(TX + 76), 0, 14)
_u.ageLbl.Position = UDim2.new(0, TX, 0, 54)
_u.ageLbl.BackgroundTransparency = 1
_u.ageLbl.Text = "Account Age: " .. tostring(LocalPlayer.AccountAge) .. " days"
_u.ageLbl.Font = Enum.Font.GothamBold; _u.ageLbl.TextSize = 10
_u.ageLbl.TextColor3 = C.sub or _C3_SUB
_u.ageLbl.TextXAlignment = Enum.TextXAlignment.Left
_u.idLbl = Instance.new("TextLabel", _u.profCard)
_u.idLbl.Size = UDim2.new(1, -(TX + 76), 0, 14)
_u.idLbl.Position = UDim2.new(0, TX, 0, 68)
_u.idLbl.BackgroundTransparency = 1
_u.idLbl.Text = "User ID: " .. tostring(LocalPlayer.UserId)
_u.idLbl.Font = Enum.Font.GothamBold; _u.idLbl.TextSize = 10
_u.idLbl.TextColor3 = C.sub or _C3_SUB
_u.idLbl.TextXAlignment = Enum.TextXAlignment.Left
_u.verF = Instance.new("Frame", _u.profCard)
_u.verF.Size = UDim2.new(0,64,0,24)
_u.verF.Position = UDim2.new(1, -76, 0, 12)
_u.verF.BackgroundColor3 = C.accent; _u.verF.BackgroundTransparency = 0.82
_u.verF.BorderSizePixel = 0; corner(_u.verF, 8)
_u.verStr = _makeDummyStroke(_u.verF)
_u.verStr.Thickness = 1; _u.verStr.Color = C.accent; _u.verStr.Transparency = 0.5
_u.verLbl = Instance.new("TextLabel", _u.verF)
_u.verLbl.Size = UDim2.new(1,0,1,0); _u.verLbl.BackgroundTransparency = 1
_u.verLbl.Text = "TLMenu"; _u.verLbl.Font = Enum.Font.GothamBlack; _u.verLbl.TextSize = 11
_u.verLbl.TextColor3 = C.accent; _u.verLbl.TextXAlignment = Enum.TextXAlignment.Center
_u.profStatPill = Instance.new("Frame", _u.profCard)
_u.profStatPill.Size = UDim2.new(0, 100, 0, 24)
_u.profStatPill.Position = UDim2.new(0, TX, 0, 88)
_u.profStatPill.BackgroundColor3 = (_TL_activeThemeId == "matrix") and Color3.fromRGB(2, 18, 8) or C.bg2
_u.profStatPill.BackgroundTransparency = 0.12
_u.profStatPill.BorderSizePixel = 0
corner(_u.profStatPill, 99)
_u.profStatPillS = _makeDummyStroke(_u.profStatPill)
_u.profStatPillS.Thickness = 1; _u.profStatPillS.Color = C.accent; _u.profStatPillS.Transparency = 0.65
_u.profDot = Instance.new("Frame", _u.profStatPill)
_u.profDot.Size = UDim2.new(0,6,0,6); _u.profDot.Position = UDim2.new(0,10,0.5,-3)
_u.profDot.BackgroundColor3 = C.accent; _u.profDot.BorderSizePixel = 0; corner(_u.profDot, 99)
_u.onLbl = Instance.new("TextLabel", _u.profStatPill)
_u.onLbl.Size = UDim2.new(1,-22,1,0); _u.onLbl.Position = UDim2.new(0,20,0,0)
_u.onLbl.BackgroundTransparency = 1; _u.onLbl.Text = T.profile_online
_u.onLbl.Font = Enum.Font.GothamBold; _u.onLbl.TextSize = 11
_u.onLbl.TextColor3 = C.accent; _u.onLbl.TextXAlignment = Enum.TextXAlignment.Left
Y = Y + PROF_CARD_H + 12
divider(Y); Y = Y + 12
local CHIP_H   = 54
local CHIP_GAP = 8
...
lbl.TextColor3 = _C3_WHITE
lbl.TextXAlignment = Enum.TextXAlignment.Left
end
sectionLbl(Y, "SYSTEM UTILS")
Y = Y + 18
_u.srvCard = Instance.new("Frame", c)
_u.srvCard.Size = UDim2.new(1, -PAD*2, 0, 140)
_u.srvCard.Position = UDim2.new(0, PAD, 0, Y)
_u.srvCard.BackgroundColor3 = C.bg2 or _C3_BG2
_u.srvCard.BorderSizePixel = 0; corner(_u.srvCard, 12)
_u.srvCardS = _makeDummyStroke(_u.srvCard)
_u.srvCardS.Thickness = 1; _u.srvCardS.Color = C.bg3 or _C3_BG3; _u.srvCardS.Transparency = 0.28
local function makeSrvBtn(x, y, w, h, txt, sub, fn)
    local b = Instance.new("TextButton", _u.srvCard)
    ...
    b.MouseEnter:Connect(function() _sc._playHoverSound(); tw(b, 0.1, {BackgroundTransparency = 0.2}):Play() end)
...
makeSrvBtn(12, 60, bw, 40, "Save Config", "Write Data", function()
    saveData(); sendNotif("Settings", "Saved ★", 2)
end)
makeSrvBtn(12 + bw + 8, 60, bw, 40, "Discord", "Join Server", function()
    pcall(function() setclipboard("https://discord.gg/tXHG8jyxpb") end)
    pcall(function() game:GetService("GuiService"):OpenBrowserWindow("https://discord.gg/tXHG8jyxpb") end)
end)
_u.rstBtn = Instance.new("TextButton", _u.srvCard)
_u.rstBtn.Position = UDim2.new(0, 12, 1, -22)
_u.rstBtn.BackgroundTransparency = 1; _u.rstBtn.Text = "Safe Reset Character"
_u.rstBtn.Font = Enum.Font.GothamBold; _u.rstBtn.TextSize = 9; _u.rstBtn.TextColor3 = C.accent
_u.rstBtn.MouseButton1Click:Connect(function() if LocalPlayer.Character then LocalPlayer.Character:BreakJoints() end end)
Y = Y + 140 + 20
...
local function makeSliderRow(yPos, label, sublabel, col, vMin, vMax, vDef, onToggle, onReset, onSlide)
    local function liveCol() return _themePanelColor(col, C.accent) end
        _u.card = Instance.new("Frame", c)
    _u.card.Size = UDim2.new(1,-PAD*2,0,CARD_H)
    _u.card.Position = UDim2.new(0,PAD,0,yPos)
    _u.card.BackgroundColor3 = C.bg2; _u.card.BackgroundTransparency = 0; _u.card.BorderSizePixel = 0
    corner(_u.card, 12)
    _u.cStr = stroke(_u.card, 1, C.bg3, 0.3)
    
    _u.cdot = Instance.new("Frame", _u.card)
    _u.cdot.Size = UDim2.new(0,3,0,CARD_H-20); _u.cdot.Visible = false
    _u.cdot.Position = UDim2.new(0,0,0.5,-(CARD_H-20)/2)
    _u.cdot.BackgroundColor3 = liveCol(); _u.cdot.BackgroundTransparency = 0.3
    _u.cdot.BorderSizePixel = 0; corner(_u.cdot, 99)
    
    _u.nameLbl = Instance.new("TextLabel", _u.card)
    _u.nameLbl.Size = UDim2.new(0,120,0,18); _u.nameLbl.Position = UDim2.new(0,14,0,8)
    _u.nameLbl.BackgroundTransparency = 1; _u.nameLbl.Text = label
    _u.nameLbl.Font = Enum.Font.GothamBold; _u.nameLbl.TextSize = 13; _u.nameLbl.TextColor3 = C.text
    _u.nameLbl.TextXAlignment = Enum.TextXAlignment.Left
    
    _u.subLbl = Instance.new("TextLabel", _u.card)
    _u.subLbl.Size = UDim2.new(0,120,0,13); _u.subLbl.Position = UDim2.new(0,14,0,26)
    _u.subLbl.BackgroundTransparency = 1; _u.subLbl.Text = sublabel
    _u.subLbl.Font = Enum.Font.GothamBold; _u.subLbl.TextSize = 9; _u.subLbl.TextColor3 = C.sub
    _u.subLbl.TextXAlignment = Enum.TextXAlignment.Left
    
    _u.valLbl = Instance.new("TextLabel", _u.card)
    _u.valLbl.Size = UDim2.new(0,52,0,18); _u.valLbl.Position = UDim2.new(1,-100,0,8)
    _u.valLbl.BackgroundTransparency = 1; _u.valLbl.Text = tostring(vDef)
    _u.valLbl.Font = Enum.Font.GothamBold; _u.valLbl.TextSize = 13; _u.valLbl.TextColor3 = liveCol()
    _u.valLbl.TextXAlignment = Enum.TextXAlignment.Left
    
    _u.rstBtn = Instance.new("TextButton", _u.card)
    _u.rstBtn.Size = UDim2.new(0,30,0,22); _u.rstBtn.Position = UDim2.new(1,-136,0,5)
    _u.rstBtn.BackgroundColor3 = C.bg3; _u.rstBtn.BackgroundTransparency = 0.2
    _u.rstBtn.Text = "R"; _u.rstBtn.Font = Enum.Font.GothamBold; _u.rstBtn.TextSize = 11; _u.rstBtn.TextColor3 = C.sub; corner(_u.rstBtn, 6)
    _u.rstStr = stroke(_u.rstBtn, 1, C.bg3, 0.4)
    
    _u.rstBtn.MouseEnter:Connect(function() twP(_u.rstBtn, 0.1, {BackgroundColor3 = liveCol(), BackgroundTransparency = 0.6}) end)
    _u.rstBtn.MouseLeave:Connect(function() twP(_u.rstBtn, 0.1, {BackgroundColor3 = C.bg3, BackgroundTransparency = 0.2}) end)
    
    _u.track = Instance.new("Frame", _u.card)
    _u.track.Size = UDim2.new(1,-28,0,4); _u.track.Position = UDim2.new(0,14,1,-14)
    _u.track.BackgroundColor3 = C.bg3; _u.track.BackgroundTransparency = 0.4; corner(_u.track, 99)
    
    _u.fill = Instance.new("Frame", _u.track)
    _u.fill.Size = UDim2.new((vDef-vMin)/(vMax-vMin),0,1,0)
    _u.fill.BackgroundColor3 = liveCol(); _u.fill.BorderSizePixel = 0; corner(_u.fill, 99)
    
    _u.knob = Instance.new("Frame", _u.track)
    _u.knob.Size = UDim2.new(0,12,0,12); _u.knob.Position = UDim2.new((vDef-vMin)/(vMax-vMin),-6,0.5,-6)
    _u.knob.BackgroundColor3 = Color3.new(1,1,1); _u.knob.ZIndex = 5; corner(_u.knob, 99)
    _u.kStr = stroke(_u.knob, 1.5, liveCol(), 0)
    ...
    task.spawn(function()
        while _u.card and _u.card.Parent do
            repeat
            local dt = task.wait()
            if not _u.card.Visible then break end
            -- Lerp
            sVisualV = sVisualV + (sTargetV - sVisualV) * math.min(dt * 22, 1)
            _u.fill.Size = UDim2.new(sVisualV,0,1,0)
            _u.knob.Position = UDim2.new(sVisualV,-6,0.5,-6)
            -- Pulse (Breathing effect on knob)
            local p = 0.65 + math.sin(os.clock()*5)*0.35
            _u.knob.BackgroundTransparency = 0.05 * p
            _u.kStr.Transparency = 0.15 * p
            until true
        end
    end)

    -- Mobile optimized interaction button (large 80x60 padding for easy grab)
    _u.sliderBtn = Instance.new("TextButton", _u.track)
    _u.sliderBtn.Size = UDim2.new(1,80,1,60); _u.sliderBtn.Position = UDim2.new(0,-40,0,-30)
    _u.sliderBtn.BackgroundTransparency = 1; _u.sliderBtn.Text = ""; _u.sliderBtn.ZIndex = 25
    
    local function upSlider(ip)
        if not _u.track or not _u.track.Parent then return end
        local ratio = math.clamp((ip.X - _u.track.AbsolutePosition.X) / _u.track.AbsoluteSize.X, 0, 1)
        applyRatio(ratio)
    end

    _u.sliderBtn.InputBegan:Connect(function(ip)
    ...
    _u.sliderBtn.InputEnded:Connect(function(ip)
        if ip.UserInputType == Enum.UserInputType.MouseButton1 or ip.UserInputType == Enum.UserInputType.Touch then
            dragging = false
        end
    end)

    _u.togTrack = Instance.new("Frame", _u.card)
    _u.togTrack.Size = UDim2.new(0, 32, 0, 18)
    _u.togTrack.Position = UDim2.new(1, -44, 0, 11)
    _u.togTrack.BackgroundColor3 = C.bg3 or _C3_BG3
    _u.togTrack.BackgroundTransparency = 0.2
    _u.togTrack.BorderSizePixel = 0
    corner(_u.togTrack, 99)
    
    _u.togKnob = Instance.new("Frame", _u.togTrack)
    _u.togKnob.Size = UDim2.new(0, 12, 0, 12)
    _u.togKnob.Position = UDim2.new(0, 2, 0.5, -6)
    _u.togKnob.BackgroundColor3 = _C3_SUB2
    _u.togKnob.BorderSizePixel = 0
    corner(_u.togKnob, 99)

    local function setToggle(on)
        togState = on
        if on then
            twP(_u.togTrack, 0.15, {BackgroundColor3 = liveCol(), BackgroundTransparency = 0.55})
            tw(_u.togKnob,  0.15, {BackgroundColor3 = _C3_WHITE, Position = UDim2.new(1,-14,0.5,-6)}):Play()
            twP(_u.cStr,     0.15, {Color = liveCol(), Transparency = 0.5})
            -- Sound bei Toggle ON
            pcall(function()
                local sound = Instance.new("Sound")
            ...
            twP(_u.togTrack, 0.15, {BackgroundColor3 = C.bg3 or _C3_BG3, BackgroundTransparency = 0.2})
            tw(_u.togKnob,  0.15, {BackgroundColor3 = _C3_SUB2, Position = UDim2.new(0,2,0.5,-6)}):Play()
            twP(_u.cStr,     0.15, {Color = C.bg3 or _C3_BG3, Transparency = 0.3})
        end
        if onToggle then onToggle(on, curVal) end
    end
    _u.togBtn = Instance.new("TextButton", _u.card)
    _u.togBtn.Size = UDim2.new(0,44,0,28); _u.togBtn.Position = UDim2.new(1,-50,0,6)
    _u.togBtn.BackgroundTransparency = 1; _u.togBtn.Text = ""; _u.togBtn.ZIndex = 7
    local _togDebounce = false
    _u.togBtn.MouseButton1Click:Connect(function()
        if _togDebounce then return end; _togDebounce = true
        setToggle(not togState)
        task.wait(0.2); _togDebounce = false
    end)
    _u.rstBtn.MouseButton1Click:Connect(function()
        applyRatio((vDef-vMin)/(vMax-vMin))
        if onReset then onReset(curVal) end
    end)
    _u.card.MouseEnter:Connect(function() _sc._playHoverSound() end)

    -- Live-Update bei Theme-Wechsel
    if _panelColorHooks then
        _panelColorHooks[#_panelColorHooks+1] = function()
            local ac = liveCol()
            pcall(function() _u.cdot.BackgroundColor3 = ac end)
            pcall(function() _u.fill.BackgroundColor3 = ac end)
            pcall(function() _u.kStr.Color            = ac end)
            pcall(function() _u.valLbl.TextColor3     = ac end)
            if togState then
                pcall(function() _u.togTrack.BackgroundColor3 = ac end)
                pcall(function() _u.cStr.Color                = ac end)
            end
        end
    end
    return _u.card, setToggle, function() return togState end
end
local TOG_H = 46
...
local function createPlayerPill(parent, yPos, labelText, defaultText, onSelect, onToggle)
    ...
    local function closeDd()
        if _activePillDropdown ~= ddFrame then return end
        _activePillDropdown = nil; setClips(true)
        ...
        t.Completed:Connect(function() if _activePillDropdown ~= ddFrame then ddFrame.Visible = false end end)
    end
    ...
    pillBtn.MouseButton1Click:Connect(function()
        if _activePillDropdown == ddFrame then closeDd(); return end
        if _activePillDropdown then pcall(function() _activePillDropdown.Visible = false end) end
        _activePillDropdown = ddFrame
    ...
    UserInputService.InputBegan:Connect(function(input)
        if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and _activePillDropdown == ddFrame then
            task.defer(function()
                local mp = UserInputService:GetMouseLocation()
                local absP, absS = ddFrame.AbsolutePosition, ddFrame.AbsoluteSize
                local inD = mp.X >= absP.X and mp.X <= absP.X + absS.X and mp.Y >= absP.Y and mp.Y <= absP.Y + absS.Y
                local pillP, pillS2 = pill.AbsolutePosition, pill.AbsoluteSize
                local inP = mp.X >= pillP.X and mp.X <= pillP.X + pillS2.X and mp.Y >= pillP.Y and mp.Y <= pillP.Y + pillS2.Y
...
local function createSelectionPill(parent, yPos, labelText, defaultText, options, onSelect, onToggle)
    local pill = Instance.new("Frame", parent)
    pill.Size = UDim2.new(1, -32, 0, 24)
    pill.Position = UDim2.new(0, 16, 0, yPos)
    pill.BackgroundColor3 = C.bg2; pill.BackgroundTransparency = 0
    ...
    local DD_IH, DD_MAX_ROWS = 32, 6
    local ddSlot = {tween = nil}
    local selectedOption = defaultText
    ...
    local function buildDd()
        ...
        for _, opt in ipairs(options) do
            ...
            optLbl.Size = UDim2.new(1, -10, 1, 0); optLbl.BackgroundTransparency = 1
            optLbl.Text = opt; optLbl.Font = Enum.Font.GothamBold; optLbl.TextSize = 13
            optLbl.TextColor3 = (selectedOption == opt) and C.red or C.text
            optLbl.TextXAlignment = Enum.TextXAlignment.Left; optLbl.ZIndex = 5003
            local rBtn = Instance.new("TextButton", row)
            rBtn.Size = UDim2.new(1, 0, 1, 0); rBtn.BackgroundTransparency = 1; rBtn.Text = ""
            rBtn.ZIndex = 5004; rBtn.Active = true
            rBtn.MouseEnter:Connect(function() twP(row, 0.15, {BackgroundTransparency = 0.7}) end)
            rBtn.MouseLeave:Connect(function() twP(row, 0.15, {BackgroundTransparency = 0.9}) end)
            rBtn.MouseButton1Click:Connect(function()
                selectedOption = opt; pillLbl.Text = opt; pillLbl.TextColor3 = C.accent
                if onSelect then onSelect(opt) end; closeDd()
            end)
        end
        local cnt = #options
        ddScroll.CanvasSize = UDim2.new(0, 0, 0, cnt * (DD_IH + 2) + 8)
                ...
                local function RunCustomAnimation(Char)
                    ...
                    local function blend2D(targetVelo, targetSpeed)
                        ...
                        for n,v1 in pairs(locomotionMap) do
                            repeat
                            if targetVelo.x * v1.lv.x < 0.0 or targetVelo.y * v1.lv.y < 0 then h[n] = 0.0; break end
                            h[n] = math.huge
                            for j,v2 in pairs(locomotionMap) do
                                repeat
                                if targetVelo.x * v2.lv.x < 0.0 or targetVelo.y * v2.lv.y < 0 then break end
...
CY = CY + TOG_H + GAP
makeToggleRow(CY, "Invisible Animation", "ghost effect", C.accent,
function(on) _invisAnimEnabled = on end, true)
CY = CY + TOG_H + 14
divider(CY); CY = CY + 14
...
local function removeOutfitFromCache(outfitId, userId)
    ...
    if entry and entry.isFolder then
        for _, so in pairs(SavedOutfitsState.outfits) do
            if so.parentFolder == tostring(outfitId) then
                so.parentFolder = nil
            end
        end
    end
    SavedOutfitsState.outfits[key] = nil
    persistSavedOutfits()
end
...
local function applyOutfit(outfitId)
    ...
    if ok then _activeOutfitId = outfitId end
    return ok
end

_tlTrackConn(LocalPlayer.CharacterAdded:Connect(function()
    if _activeOutfitId then
        applyOutfit(_activeOutfitId)
    end
end))
...
local function tweenOpen(frame, w, h)
    ...
    s.Scale = 0.94
    -- Snappy pop-in with ultra-fast Back bounce
    tween(s, TweenInfo.new(0.12, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1}):Play()
end

local function tweenClose(frame, w, h, cb)
    local s = getPopScale(frame)
    -- Snappy pop-out
    tween(s, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {Scale = 0.94}):Play()
...
local function makeCloseBtn(parent, size, posX, posY, anchorX, anchorY)
    local actualSize = _isMobile and (size + 6) or size
    local btn = Instance.new("ImageButton")
    btn.Size                  = UDim2.fromOffset(actualSize, actualSize)
    ...
    bind(btn.MouseButton1Down, function() tween(btn, TI._008, { Size=UDim2.fromOffset(actualSize-1,actualSize-1) }):Play() end)
    bind(btn.MouseButton1Up,   function() tween(btn, TI._008, { Size=UDim2.fromOffset(actualSize,actualSize) }):Play() end)
    return btn
end
...
local Panel, TitleBar, SearchBox, ScrollFrame, LoadingLabel, SavedOutfitsBtn, RefreshBtn, CloseBtn = (function()
    local Panel = Instance.new("Frame")
    _TL_refs.Panel = Panel
    Panel.Name                   = "Panel"
    ...
    TitleIcon.ScaleType = Enum.ScaleType.Fit; TitleIcon.Image = "rbxassetid://125139667304157"; TitleIcon.Size = UDim2.fromOffset(24,24); TitleIcon.Position = UDim2.fromOffset(6,10)
    TitleIcon.BackgroundTransparency = 1; TitleIcon.ImageColor3 = Color3.fromRGB(255, 255, 255)
    TitleIcon.ZIndex = 21; TitleIcon.Parent = TitleBar

    local TitleLabel = Instance.new("TextLabel")
    TitleLabel.Text = "TLSTEAL AVATARS"; TitleLabel.Size = UDim2.new(1,-158,1,0)
    ...
    TitleLabel.ZIndex = 21; TitleLabel.Parent = TitleBar; applyTextStyle(TitleLabel)
    ...
    local padding = 4
    local btnSize = _isMobile and 32 or 26
    local posRefresh = -38 - padding
    local posSaved = posRefresh - btnSize - padding

    local SavedOutfitsBtn = Instance.new("ImageButton")
    SavedOutfitsBtn.Size = UDim2.fromOffset(btnSize,btnSize); SavedOutfitsBtn.Position = UDim2.new(1, posSaved, 0.5, 0)
    SavedOutfitsBtn.AnchorPoint = Vector2.new(1, 0.5); SavedOutfitsBtn.BackgroundTransparency = 1
    SavedOutfitsBtn.Image = "rbxassetid://12975878363"; SavedOutfitsBtn.ImageColor3 = Color3.fromRGB(255,255,255)
    SavedOutfitsBtn.ZIndex = 14; SavedOutfitsBtn.Parent = TitleBar; corner(SavedOutfitsBtn, 6)
    bind(SavedOutfitsBtn.MouseEnter, function() tween(SavedOutfitsBtn, TI._012, {ImageColor3=Color3.fromRGB(200,200,200), Size=UDim2.fromOffset(btnSize+1,btnSize+1)}):Play() end)
    bind(SavedOutfitsBtn.MouseLeave, function() tween(SavedOutfitsBtn, TI._012, {ImageColor3=Color3.fromRGB(255,255,255), Size=UDim2.fromOffset(btnSize,btnSize)}):Play() end)
    ...
    RefreshBtn.Size = UDim2.fromOffset(btnSize,btnSize); RefreshBtn.Position = UDim2.new(1, posRefresh, 0.5, 0)
    RefreshBtn.AnchorPoint = Vector2.new(1, 0.5); RefreshBtn.BackgroundTransparency = 1
    RefreshBtn.Image = "rbxassetid://137689074320233"; RefreshBtn.ImageColor3 = Color3.fromRGB(255,255,255)
    RefreshBtn.Parent = TitleBar; corner(RefreshBtn, 6)
    bind(RefreshBtn.MouseEnter, function() tween(RefreshBtn, TI._012, {ImageColor3=Color3.fromRGB(200,200,200), Size=UDim2.fromOffset(btnSize+1,btnSize+1)}):Play() end)
    bind(RefreshBtn.MouseLeave, function() tween(RefreshBtn, TI._012, {ImageColor3=Color3.fromRGB(255,255,255), Size=UDim2.fromOffset(btnSize,btnSize)}):Play() end)
    ...
    local CloseBtn = makeCloseBtn(TitleBar, 26, 1, -38, 0.5, 0); CloseBtn.ZIndex = 14
    ...
    SearchPill.ZIndex = 12; SearchPill.Parent = Panel; corner(SearchPill, 8)
    ...
    SearchIcon.Image = "rbxassetid://109414459999540"; SearchIcon.Size = UDim2.fromOffset(26,26)
    SearchIcon.Position = UDim2.fromOffset(13,4); SearchIcon.BackgroundTransparency = 1
    SearchIcon.ImageColor3 = Color3.fromRGB(255,255,255); SearchIcon.ImageTransparency = 0
    SearchIcon.ZIndex = 13; SearchIcon.Parent = SearchPill
    ...
    bind(SearchBox.Focused, function()
        ...
        tween(SearchIcon, TI._008, {ImageColor3 = C.accent or Color3.fromRGB(140,140,255), ImageTransparency = 0}):Play()
    end)
    bind(SearchBox.FocusLost, function()
        ...
        tween(SearchIcon, TI._012, {ImageColor3 = Color3.fromRGB(255,255,255), ImageTransparency = 0}):Play()
    end)
    ...
    -- (PlayerCountLabel removed)
    setupDraggable(Panel, TitleBar, 0)
    return Panel, TitleBar, SearchBox, ScrollFrame, LoadingLabel, SavedOutfitsBtn, RefreshBtn, CloseBtn
end)()
...
local SubPanel, SubTitleBar, SubTitleName, SubTitleSub, SubAvatarThumb, SubCloseBtn, SubBackBtn, SubRefreshBtn, SubPageContainer, SubPagePrev, SubPageNext, SubPageLabel, OutfitScroll, OutfitLoading, OutfitEmpty, OutfitReloadBtn, OutfitReloadLabel = (function()
    ...
    local SubCloseBtn = makeCloseBtn(SubTitleBar, 26, 1, -38, 0.5, 0); SubCloseBtn.ZIndex = 23

    local paddingSub = 4
    local btnSize = _isMobile and 32 or 26
    local posSubBack = -38 - paddingSub
    local posSubRefresh = posSubBack - btnSize - paddingSub

    local SubBackBtn = Instance.new("ImageButton")
    SubBackBtn.Size = UDim2.fromOffset(btnSize,btnSize); SubBackBtn.Position = UDim2.new(1, posSubBack, 0.5, 0)
    SubBackBtn.AnchorPoint = Vector2.new(1, 0.5); SubBackBtn.BackgroundTransparency = 1
    SubBackBtn.ScaleType = Enum.ScaleType.Fit; SubBackBtn.Image = "rbxassetid://93168176661050"; SubBackBtn.ImageColor3 = Color3.fromRGB(255, 255, 255)
    SubBackBtn.ZIndex = 23; SubBackBtn.Parent = SubTitleBar; corner(SubBackBtn, 6)
    bind(SubBackBtn.MouseEnter, function() tween(SubBackBtn, TI._012, {ImageColor3=Color3.fromRGB(200, 200, 200)}):Play() end)
    bind(SubBackBtn.MouseLeave, function() tween(SubBackBtn, TI._012, {ImageColor3=Color3.fromRGB(255, 255, 255)}):Play() end)
    ...
    SubRefreshBtn.Name = "SubRefreshBtn"; SubRefreshBtn.Size = UDim2.fromOffset(btnSize,btnSize)
    SubRefreshBtn.Position = UDim2.new(1, posSubRefresh, 0.5, 0); SubRefreshBtn.AnchorPoint = Vector2.new(1, 0.5)
    SubRefreshBtn.BackgroundTransparency = 1; SubRefreshBtn.BorderSizePixel = 0
    SubRefreshBtn.ScaleType = Enum.ScaleType.Fit; SubRefreshBtn.Image = "rbxassetid://137689074320233"; SubRefreshBtn.ImageColor3 = Color3.fromRGB(255,255,255)
    SubRefreshBtn.ZIndex = 22; SubRefreshBtn.Parent = SubTitleBar
    corner(SubRefreshBtn, 5)
    ...
    corner(SubPagePrev, 5); local spStroke1 = _makeDummyStroke(SubPagePrev); spStroke1.Color = C.bg3 or Color3.fromRGB(45,45,45)
    ...
    corner(SubPageNext, 5); local spStroke2 = _makeDummyStroke(SubPageNext); spStroke2.Color = C.bg3 or Color3.fromRGB(45,45,45)
...
local openSavedPanel, appendSavedOutfitCard, currentSavedFolderKey, pickingForFolder, SavedFolderBtn, SavedRenameBtn
...
SavedPanel, SavedTitleBar, SavedTitleName, SavedTitleSub, SavedCloseBtn, SavedScroll, SavedEmpty, SavedBackBtn, SavedFolderBtn, SavedRenameBtn, SavedIcon = (function()
    ...
    SavedIcon = Instance.new("TextLabel")
    ...
    local SavedBackBtn = Instance.new("ImageButton")
    local padding = 4
    local btnSize = _isMobile and 32 or 26
    local posBack = -38 - padding
    local posFolder = posBack - btnSize - padding
    local posRename = posFolder - btnSize - padding
    
    SavedBackBtn.Size = UDim2.fromOffset(btnSize,btnSize); SavedBackBtn.Position = UDim2.new(1, posBack, 0.5, 0)
    SavedBackBtn.AnchorPoint = Vector2.new(1, 0.5); SavedBackBtn.BackgroundTransparency = 1; corner(SavedBackBtn, 6)
    SavedBackBtn.ScaleType = Enum.ScaleType.Fit; SavedBackBtn.Image = "rbxassetid://93168176661050"; SavedBackBtn.ImageColor3 = Color3.fromRGB(255, 255, 255)
    SavedBackBtn.ZIndex = 28; SavedBackBtn.Visible = true; SavedBackBtn.Parent = SavedTitleBar
    bind(SavedBackBtn.MouseEnter, function() tween(SavedBackBtn, TI._012, {ImageColor3=Color3.fromRGB(200, 200, 200)}):Play() end)
    bind(SavedBackBtn.MouseLeave, function() tween(SavedBackBtn, TI._012, {ImageColor3=Color3.fromRGB(255, 255, 255)}):Play() end)
    bind(SavedBackBtn.MouseButton1Click, function()
        if pickingForFolder then
            pickingForFolder = nil
            if _TL_refs.openSavedPanel then _TL_refs.openSavedPanel() end
        elseif currentSavedFolderKey then
            currentSavedFolderKey = nil
            if _TL_refs.openSavedPanel then _TL_refs.openSavedPanel() end
        else
            if _TL_refs.closeSavedPanel then _TL_refs.closeSavedPanel() end
        end
    end)

    local SavedTitleName = Instance.new("TextLabel")
    SavedTitleName.Text = T.saved_outfits_title; SavedTitleName.Size = UDim2.new(1,-150,0,22)
    ...
    SavedTitleSub.Text = T.saved_outfits_sub; SavedTitleSub.Size = UDim2.new(1,-150,0,16)
    ...
    local SavedCloseBtn = makeCloseBtn(SavedTitleBar, 26, 1, -38, 0.5, 0); SavedCloseBtn.ZIndex = 28
    SavedFolderBtn = Instance.new("ImageButton")
    SavedFolderBtn.Size = UDim2.fromOffset(btnSize,btnSize)
    SavedFolderBtn.Position = UDim2.new(1, posFolder, 0.5, 0)
    SavedFolderBtn.AnchorPoint = Vector2.new(1, 0.5)
    SavedFolderBtn.BackgroundTransparency = 1
    SavedFolderBtn.ScaleType = Enum.ScaleType.Fit; SavedFolderBtn.Image = "rbxassetid://135828379467778"
    SavedFolderBtn.ImageColor3 = Color3.fromRGB(255, 255, 255)
    SavedFolderBtn.ZIndex = 28
    SavedFolderBtn.Parent = SavedTitleBar
    bind(SavedFolderBtn.MouseEnter, function() tween(SavedFolderBtn, TI._012, {ImageColor3=Color3.fromRGB(200, 200, 200)}):Play() end)
    bind(SavedFolderBtn.MouseLeave, function() tween(SavedFolderBtn, TI._012, {ImageColor3=Color3.fromRGB(255, 255, 255)}):Play() end)
    bind(SavedFolderBtn.MouseButton1Down, function() tween(SavedFolderBtn, TI._008, {Size=UDim2.fromOffset(btnSize-1,btnSize-1)}):Play() end)
    bind(SavedFolderBtn.MouseButton1Up, function() tween(SavedFolderBtn, TI._008, {Size=UDim2.fromOffset(btnSize,btnSize)}):Play() end)
    bind(SavedFolderBtn.MouseButton1Click, function()
        local fid = "FOLDER_" .. tostring(math.floor(tick()))
        local fname = T.new_folder
        local key = "_" .. fid
        if not SavedOutfitsState.loaded then loadSavedOutfitsFromCache(true) end
        SavedOutfitsState.outfits[key] = { outfitId=fid, outfitName=fname, isFolder=true, contents={}, userId="" }
        persistSavedOutfits()
        if _TL_refs.openSavedPanel then _TL_refs.openSavedPanel() end
    end)

    SavedRenameBtn = Instance.new("ImageButton")
    SavedRenameBtn.Size = UDim2.fromOffset(btnSize,btnSize)
    SavedRenameBtn.Position = UDim2.new(1, posFolder, 0.5, 0)
    SavedRenameBtn.AnchorPoint = Vector2.new(1, 0.5)
    SavedRenameBtn.BackgroundTransparency = 1
    SavedRenameBtn.ScaleType = Enum.ScaleType.Fit; SavedRenameBtn.Image = "rbxassetid://74424838226861"
    SavedRenameBtn.ImageColor3 = Color3.fromRGB(255, 255, 255)
    SavedRenameBtn.ZIndex = 50
    SavedRenameBtn.Visible = false
    SavedRenameBtn.Parent = SavedTitleBar; corner(SavedRenameBtn, 6)
    bind(SavedRenameBtn.MouseEnter, function() tween(SavedRenameBtn, TI._012, {ImageColor3=Color3.fromRGB(200, 200, 200)}):Play() end)
    bind(SavedRenameBtn.MouseLeave, function() tween(SavedRenameBtn, TI._012, {ImageColor3=Color3.fromRGB(255, 255, 255)}):Play() end)
    bind(SavedRenameBtn.MouseButton1Down, function() tween(SavedRenameBtn, TI._008, {Size=UDim2.fromOffset(btnSize-1,btnSize-1)}):Play() end)
    bind(SavedRenameBtn.MouseButton1Up, function() tween(SavedRenameBtn, TI._008, {Size=UDim2.fromOffset(btnSize,btnSize)}):Play() end)
    bind(SavedRenameBtn.MouseButton1Click, function()
        if not currentSavedFolderKey then return end
        local fKey = "_"..currentSavedFolderKey
        local fData = SavedOutfitsState.outfits[fKey] or SavedOutfitsState.outfits[currentSavedFolderKey]
        if not fData then return end
        
        SavedTitleName.Visible = false; SavedTitleSub.Visible = false
        
        local eb = Instance.new("TextBox")
        eb.Size = UDim2.new(1, -120, 0, 28); eb.Position = UDim2.new(0, 42, 0, 25)
        eb.AnchorPoint = Vector2.new(0, 0.5); eb.Text = fData.outfitName
        eb.BackgroundColor3 = Color3.fromRGB(25, 25, 30); eb.TextColor3 = Color3.new(1, 1, 1)
        eb.BorderSizePixel = 0; eb.ZIndex = 60; eb.Parent = SavedPanel
        corner(eb, 6); applyTextStyle(eb); eb:CaptureFocus()
        eb.FocusLost:Connect(function()
            if eb.Text ~= "" and eb.Text ~= fData.outfitName then
                fData.outfitName = eb.Text
                persistSavedOutfits()
                if _TL_refs.openSavedPanel then _TL_refs.openSavedPanel() end
            end
            eb:Destroy()
            SavedTitleName.Visible = true; SavedTitleSub.Visible = true
        end)
    end)
    ...
    SavedGrid.SortOrder = Enum.SortOrder.LayoutOrder; SavedGrid.HorizontalAlignment = Enum.HorizontalAlignment.Center
    SavedGrid.Parent = SavedScroll

    local SavedGridPad = Instance.new("UIPadding")
    SavedGridPad.PaddingLeft = UDim.new(0,0); SavedGridPad.PaddingTop = UDim.new(0,6)
...
local function nextThumbRenderToken()   activeThumbRenderToken = activeThumbRenderToken + 1; return activeThumbRenderToken end
local function invalidateThumbRenderToken() activeThumbRenderToken = activeThumbRenderToken + 1 end
local function nextPlayerListToken()    activePlayerListToken = activePlayerListToken + 1; return activePlayerListToken end
...
local function attachOutfitThumbnail(thumb, outfitId, renderToken, loadDelay)
    ...
    -- Validate token at call time (not after delay) so stagger doesn't cancel valid loads
    if renderToken and activeThumbRenderToken ~= renderToken then return end
    local url = outfitThumbUrl(outfitId)
    if url == "" then return end

    -- Bypassing ContentProvider:PreloadAsync and loadDelay.
    -- Directly assigning the image is significantly faster as Roblox resolves and caches them natively asynchronously.
    thumb.Image = url
    thumb.Visible = true
...
local function resortSavedOutfitCards()
    ...
    table.sort(cards, function(a, b)
        local aBack = a:GetAttribute("IsBackAction") and 1 or 0
        local bBack = b:GetAttribute("IsBackAction") and 1 or 0
        if aBack ~= bBack then return aBack > bBack end
        
        local aAdd = a:GetAttribute("IsAddAction") and 1 or 0
        local bAdd = b:GetAttribute("IsAddAction") and 1 or 0
        if aAdd ~= bAdd then return aAdd > bAdd end
        
        local aFol = a:GetAttribute("IsFolder") and 1 or 0
        local bFol = b:GetAttribute("IsFolder") and 1 or 0
        if aFol ~= bFol then return aFol > bFol end
        
        local aAt = a:GetAttribute("SavedAt") or 0
        local bAt = b:GetAttribute("SavedAt") or 0
        if aAt ~= bAt then return aAt > bAt end -- Neueste oben
...
local function createOutfitCard(parent, idx, outfitName, outfitId, isSavedPanel, savedOutfitData, renderToken, thumbLoadDelay)
    ...
    if isSavedPanel and savedOutfitData then
        ...
        card:SetAttribute("SavedKey", skey)
        card:SetAttribute("SortName", tostring(outfitName or "Unnamed"))
        card:SetAttribute("IsFolder", savedOutfitData.isFolder == true)
        card:SetAttribute("IsAddAction", savedOutfitData.isAddAction == true)
        card:SetAttribute("IsBackAction", savedOutfitData.isBackAction == true)
        card:SetAttribute("SavedAt", savedOutfitData.savedAt or 0)
    end
    _TL_refs.appendSavedOutfitCard = appendSavedOutfitCard
    ...
    if savedOutfitData and savedOutfitData.isBackAction then
        thumb.Image = "rbxassetid://100634669779590"
        thumb.ImageTransparency = 0
        thumb.BackgroundColor3 = Color3.fromRGB(18, 18, 20)
        thumb.ScaleType = Enum.ScaleType.Fit
    elseif savedOutfitData and savedOutfitData.isAddAction then
        thumb.Image = "rbxassetid://120703890568713"
        thumb.ImageTransparency = 0
        thumb.BackgroundColor3 = Color3.fromRGB(18, 18, 20)
        thumb.ScaleType = Enum.ScaleType.Fit
    elseif savedOutfitData and savedOutfitData.isFolder then
        thumb.Image = "rbxassetid://123514430148126"
        thumb.ImageTransparency = 0
        thumb.BackgroundColor3 = Color3.fromRGB(18, 18, 20)
        thumb.ScaleType = Enum.ScaleType.Fit
        thumb.ImageColor3 = C.ACCENT
    else
    ...
    if savedOutfitData and savedOutfitData.isFolder then
        local ren = Instance.new("ImageButton")
        ren.Size = UDim2.fromOffset(18,18); ren.Position = UDim2.new(1,-24,0,4)
        ren.Image = "rbxassetid://13476104880"; ren.BackgroundTransparency = 1; ren.ZIndex = 26; ren.Parent = card
        bind(ren.MouseButton1Click, function()
            local eb = Instance.new("TextBox")
            eb.Size = nameL.Size; eb.Position = nameL.Position; eb.Text = nameL.Text
            eb.BackgroundColor3 = Color3.fromRGB(30,30,35); eb.TextColor3 = Color3.new(1,1,1)
            eb.BorderSizePixel = 0; eb.ZIndex = 30; eb.Parent = card; eb:CaptureFocus()
            eb.FocusLost:Connect(function(enter)
                if enter and eb.Text ~= "" then
                    savedOutfitData.outfitName = eb.Text
                    persistSavedOutfits(); if _TL_refs.openSavedPanel then _TL_refs.openSavedPanel() end
                end
                eb:Destroy()
            end)
        end)
    ...
    if not isSavedPanel then
        ...
        saveBtn.BorderSizePixel = 0; saveBtn.ZIndex = 26; saveBtn.Parent = card; corner(saveBtn, 5)
    ...
    elseif savedOutfitData and savedOutfitData.isAddAction then
        bind(hbtn.MouseButton1Click, function() 
            pickingForFolder = currentSavedFolderKey
            if _TL_refs.openSavedPanel then _TL_refs.openSavedPanel() end
        end)
    elseif savedOutfitData and savedOutfitData.isFolder then
        bind(hbtn.MouseButton1Click, function() 
            if pickingForFolder then return end -- Don't move folders into folders for now
            currentSavedFolderKey = outfitId
            if _TL_refs.openSavedPanel then _TL_refs.openSavedPanel() end
        end)
    else
        bind(hbtn.MouseButton1Click, function() 
            if pickingForFolder then
                savedOutfitData.parentFolder = pickingForFolder
                pickingForFolder = nil
                persistSavedOutfits()
                if _TL_refs.openSavedPanel then _TL_refs.openSavedPanel() end
            else
...
function appendSavedOutfitCard(savedOutfitData)
    ...
    -- Nur appenden, wenn wir gerade im Main-Panel sind (kein Ordner offen)
    if (currentSavedFolderKey ~= nil) or (pickingForFolder ~= nil) then
        refreshSavedPanelHeader()
        return
    end
...
local function closeOutfitPanel()
    ...
    if _TL_refs.Panel then _TL_refs.Panel.Visible = true end
end

_TL_refs.closeOutfitPanel = closeOutfitPanel
...
_TL_refs.closeSavedPanel = closeSavedPanel

function openSavedPanel()
    if _TL_refs.Panel then 
        SavedPanel.Position = _TL_refs.Panel.Position
        _TL_refs.Panel.Visible = false 
    end
    _TL_refs.openSavedPanel = openSavedPanel
    
    -- Set header content BEFORE opening to avoid pop-in delay
    ...
    SavedTitleSub.Text = (currentSavedFolderKey ~= nil) and "Ordneransicht" or "Wird geladen..."
    SavedBackBtn.Visible = true
    SavedIcon.Visible = true
    ...
    for _, so in pairs(SavedOutfitsState.outfits) do
        local show = false
        if pickingForFolder then
            if not so.isFolder and not so.parentFolder then show = true end
        elseif currentSavedFolderKey == nil then
            if so.isFolder or not so.parentFolder then show = true end
        else
            if so.parentFolder == currentSavedFolderKey then show = true end
        end
        if show then
    ...
    table.sort(savedList, function(a,b) 
        local aD = a.savedOutfitData
        local bD = b.savedOutfitData
        local aS = aD.isBackAction and 3 or (aD.isAddAction and 2 or (aD.isFolder and 1 or 0))
        local bS = bD.isBackAction and 3 or (bD.isAddAction and 2 or (bD.isFolder and 1 or 0))
        if aS ~= bS then return aS > bS end
        
        local aAt = aD.savedAt or 0
        local bAt = bD.savedAt or 0
        if aAt ~= bAt then return aAt > bAt end -- Neueste oben
    ...
    if #savedList > 0 or currentSavedFolderKey ~= nil then
        ...
        -- Update subtext with count after loading
        if currentSavedFolderKey == nil then
            SavedTitleSub.Text = tostring(countSavedOutfits()) .. " Outfit(s)"
        end
    end
    
    if SavedFolderBtn then
        SavedFolderBtn.Visible = (currentSavedFolderKey == nil and pickingForFolder == nil)
    end
    if SavedRenameBtn then
        SavedRenameBtn.Visible = (currentSavedFolderKey ~= nil and pickingForFolder == nil)
    end
    
    if pickingForFolder then
        SavedTitleName.Text = "Wähle Outfit zum Hinzufügen..."
        SavedIcon.Visible = false
        SavedBackBtn.Visible = true
    elseif currentSavedFolderKey then
        local fData = SavedOutfitsState.outfits["_"..currentSavedFolderKey] or SavedOutfitsState.outfits[currentSavedFolderKey]
        SavedTitleName.Text = T.folder_prefix .. (fData and fData.outfitName or T.unknown)
        SavedIcon.Visible = false
        SavedBackBtn.Visible = true
    else
        SavedTitleName.Text = "Saved Outfits"
        SavedIcon.Visible = true
        SavedBackBtn.Visible = true
    end
    _TL_refs.openSavedPanel = openSavedPanel
...
local function openOutfitPanel(player)
    if _TL_refs.Panel then 
        SubPanel.Position = _TL_refs.Panel.Position
        _TL_refs.Panel.Visible = false 
    end
    ...
    OutfitEmpty.TextTransparency = 1; OutfitEmpty.Text = T.no_outfits_saved
    OutfitReloadBtn.Visible = false; OutfitReloadLabel.Visible = false
...
local function syncPlayerCardsIncremental()
    ...
    if #all == 0 and (playerSearchTerm or "") ~= "" then
        LoadingLabel.Text = "✕  Couldnt find \"" .. playerSearchTerm .. "\""
...
local function closePanel()
    isOpen = false
    activePlayerListToken = activePlayerListToken + 1
...
local function schedulePlayerListRefresh(delaySeconds)
    scheduledPlayerRefreshToken = scheduledPlayerRefreshToken + 1
        ...
        if hrp then
            ...
            if _ui_lastPos then
                ...
                if jump > MAX_JUMP then
                    if os.clock() - (_ui_lastTP or 0) > 0.5 then
...
movePage.Size = UDim2.new(1, 0, 0, 336) -- 6 Rows (Anti-Void, UI, Anti-Ragdoll, Punch-Fling, Touch-Fling, Click-Teleport)
...
do
    ...
    task.spawn(function()
        while task.wait(0.25) do
            repeat
            if not tfActive then break end
            pcall(function()
                for _, p in ipairs(Players:GetPlayers()) do
                    if p ~= LocalPlayer and p.Character then
                        for _, part in ipairs(p.Character:GetChildren()) do
                            ...
                            elseif part:IsA("Accessory") or part:IsA("Tool") then
                                for _, innerPart in ipairs(part:GetChildren()) do
                                    if innerPart:IsA("BasePart") then innerPart.CanCollide = false end
                                end
                            end
                        end
                    end
    ...
    _tlTrackConn(RunService.RenderStepped:Connect(function()
    ...
    task.spawn(function()
        while sTrack and sTrack.Parent do
            repeat
            local dt = task.wait()
            if not movePage.Visible then break end
...
_sc.resizeScriptsPanel = function(contentH, easeStyle, easeDir, duration)
    ...
    _sc.panelTw.p   = twP(p,        dur, {Size = UDim2.new(0, PANEL_W, 0, newH)},    sty, dir)
...
_sc.folderHdrH = 40
_sc.makeMiscFolder = function(folderName, folderIcon, accentCol, layoutOrder, pageParent)
    local isOpen = false
...
local combatContainer, combatContent, combatAddRow = _sc.makeMiscFolder("Combat Tools", "TL", C.red, 1, combatPage)
...
_aim.Players = game:GetService("Players")
_aim.RunService = game:GetService("RunService")
_aim.UserInputService = game:GetService("UserInputService")
_aim.Workspace = game:GetService("Workspace")
_aim.LocalPlayer = _aim.Players.LocalPlayer
...
_aim.Config = {
    ...
    Smoothness = 0.15,
    FOV = 120,
    AimPart = "Head", -- "Head", "Torso", "Random", "Smart"
    AutoFire = false,
    TriggerBot = false,
    MultiPoint = false,
...
-- Aimbot Cache (Per-Player)
_aim.Cache = {}

-- Utility Functions
_aim.GetDeltaTime = function()
    return os.clock() -- More precise than tick()
end
...
_aim.GetAimPart = function(character)
    ...
    -- Smart selection: Prioritize visible parts
...
_aim.GetTargetPosition = function(player, targetPart, dt)
    ...
    -- Calculate player-specific smoothed velocity
    ...
    if rootPart then
        if not _aim.Cache[player] then
            ...
            if deltaTime > 0 then
                ...
                -- Exponential Moving Average for velocity smoothing (Master Precision)
            ...
            velocity = cache.SmoothedVelocity
        end
    end
    
    -- MultiPoint: Micro-oscillations to bypass some hit-reg checks
...
_aim.GetBestTarget = function()
    ...
    local bestScore = math.huge -- Hybrid score (Screen + World)
    ...
    for _, player in ipairs(_aim.Players:GetPlayers()) do
        repeat
        if player == _aim.LocalPlayer then break end
        if not _aim.IsPlayerAlive(player) then break end
        if _aim.IsTeammate(player) then break end
        ...
        if worldDist > _aim.Config.AutoAimDistance then break end
        
        local targetPart = _aim.GetAimPart(character)
        if not targetPart then break end
        
        if _aim.Config.WallCheck and not _aim.IsVisible(targetPart) then break end
        ...
        if distFromCenter > _aim.Config.FOV then break end
...
_aim.PerformAimbot = function(dt)
    ...
    if _aim.Config.CurrentTarget then
        ...
        if player and player.Parent and _aim.IsPlayerAlive(player) then
            ...
            -- Immediate visibility and range re-check
            if part and (not _aim.Config.WallCheck or _aim.IsVisible(part)) then
                ...
                -- Dynamic lock leniency: 2x FOV for existing target
    ...
    -- Find new target if current is lost or invalid
    if not target then
    ...
    if target and target.Part then
        ...
        if targetPosition then
            -- Visuals: Target Line
            if _aim.Config.ShowTargetLine then
            ...
            -- Auto Fire Logic
            if _aim.Config.AutoFire and not _aim.Config.TriggerBot then
                local screenPos, onScreen = _aim.Camera:WorldToViewportPoint(targetPosition)
                if onScreen then
                    local distToTarget = (Vector2.new(screenPos.X, screenPos.Y) - _aim.UserInputService:GetMouseLocation()).Magnitude
                    if distToTarget < 25 then
                        -- Game-specific fire trigger could go here
                    end
                end
            end
        end
...
_aim.StartTriggerBot = function()
    ...
    _aim.TriggerBotConnection = _aim.RunService.Heartbeat:Connect(function()
        ...
        -- Performance Fix: TriggerBot has no shooting logic yet, so calling GetBestTarget()
        -- every frame causes unnecessary lag. Commented out until implemented.
...
do
    ...
    combatAddRow("Wall Check", "Combat", C.red, true, function(on)
        _aim.Config.WallCheck = on
    end)

    combatAddRow("Show FOV Circle", "Combat", C.red, true, function(on)
        _aim.Config.ShowFOV = on
    end)

    combatAddRow("Show Target Line", "Combat", C.red, true, function(on)
...
;(function()
    ...
    fovVal.BackgroundTransparency = 1; fovVal.Text = tostring(_aim.Config.FOV); fovVal.Font = Enum.Font.GothamBold
    ...
    local fovPct = (_aim.Config.FOV - 30) / 270
    ...
    local function fovUpdateFromMouse(x)
        ...
        _aim.Config.FOV = 30 + math.floor(p * 270)
        fovVal.Text = tostring(_aim.Config.FOV)
        _aim.FOVCircle.Radius = _aim.Config.FOV
    end
    fovTrack.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then fovDrag = true; fovUpdateFromMouse(i.Position.X) end end)
    UserInputService.InputChanged:Connect(function(i) if fovDrag and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then fovUpdateFromMouse(i.Position.X) end end)
    UserInputService.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then fovDrag = false end end)
    -- Hover Effects
    fovRow.MouseEnter:Connect(function() _sc._playHoverSound(); game:GetService("TweenService"):Create(fovRow, TweenInfo.new(0.08), {BackgroundColor3 = C.bg3 or Color3.fromRGB(55,55,65)}):Play() end)
    ...
    smVal.BackgroundTransparency = 1; smVal.Text = string.format("%.2f", _aim.Config.Smoothness); smVal.Font = Enum.Font.GothamBold
    ...
    local smPct = (_aim.Config.Smoothness - 0.01) / 0.19
    ...
    local function smUpdateFromMouse(x)
        ...
        _aim.Config.Smoothness = 0.01 + (p * 0.19)
        smVal.Text = string.format("%.2f", _aim.Config.Smoothness)
    end
    smTrack.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then smDrag = true; smUpdateFromMouse(i.Position.X) end end)
    UserInputService.InputChanged:Connect(function(i) if smDrag and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then smUpdateFromMouse(i.Position.X) end end)
    UserInputService.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then smDrag = false end end)
    -- Hover Effects
    smRow.MouseEnter:Connect(function() _sc._playHoverSound(); game:GetService("TweenService"):Create(smRow, TweenInfo.new(0.08), {BackgroundColor3 = C.bg3 or Color3.fromRGB(55,55,65)}):Play() end)
...
LocalPlayer.CharacterRemoving:Connect(function()
    -- No longer stopping Aimbot on character removal to keep it persistent across respawns
    -- Only stop TriggerBot if needed, but usually we want to keep it too
    -- StopTriggerBot() 
    pcall(function() if _aim.FOVCircle.Remove then _aim.FOVCircle:Remove() elseif _aim.FOVCircle.Destroy then _aim.FOVCircle:Destroy() end end)
    pcall(function() if _aim.TargetLine.Remove then _aim.TargetLine:Remove() elseif _aim.TargetLine.Destroy then _aim.TargetLine:Destroy() end end)
end)
...
local _miscScripts = {
    ...
    { id="99Nights",  icon="", col=Color3.fromRGB(80, 200, 100), label="99 Nights", baseCol=Color3.fromRGB(80, 200, 100) },
}
...
local function _miscSwitchTo(sc)
    ...
    _miscSegApplyVisual()
    task.defer(_sc.updateMiscSize)
end

local _miscSegWrap = Instance.new("Frame", _sc.sonstigePage)
...
-- -- Bladeball (Panel) -----------------------------------------------------
local bbContainer, bbContent, bbAddRow, _bbFolderApi = _sc.makeMiscGamePanel(C.accent2, 1)
-- -------------------------------------------------------------------------
local apRow, apSetFn
...
;(function() -- MM2: eigener Funktions-Scope – eigenes Register-Limit (_sc.makeMiscFolder innen – äußeres Local-Limit)
local mm2Container, mm2Content, mm2AddRow, _mm2FolderApi = _sc.makeMiscGamePanel(C.accent, 2)
...
local function _mm2UpdateESP()
    ...
    for _, player in ipairs(_mm2Players:GetPlayers()) do
        repeat
        if player == _mm2LP then break end
        local char = player.Character
...
local function _mm2GetAllTargets()
    ...
    for _, player in ipairs(_mm2Players:GetPlayers()) do
        repeat
        if player == _mm2LP then break end
        local char = player.Character; if not char then break end
...
local function _mm2ToggleKA(on)
    ...
    _mm2KAConn = { _thread = task.spawn(function()
        while _mm2Cfg.KillAura.Enabled do
            ...
            local hrp  = char and char:FindFirstChild("HumanoidRootPart")
            local hum  = char and char:FindFirstChildOfClass("Humanoid")
            if not hrp or not hum or hum.Health <= 0 then task.wait(0.2); break end

            local weapon, equipped = _mm2GetWeapon()
            if not weapon then task.wait(0.2); break end
            _mm2EquipWeapon(weapon, equipped)

            local targets = _mm2GetAllTargets()
            if #targets == 0 then task.wait(0.3); break end

            for _, target in ipairs(targets) do
                repeat
                if not _mm2Cfg.KillAura.Enabled then break end
                local tChar = target.Character; if not tChar then break end
                ...
                if not tChar.Parent or tHum.Health <= 0 then break end
...
-- -------------------------------------------------------------------------
-- -- Da Hood Ordner --------------------------------------------------------
;(function() -- DaHood: eigener Funktions-Scope (_sc.makeMiscFolder hier – weniger Locals im äußeren IIFE)
local dhContainer, dhContent, dhAddRow, _dhFolderApi = _sc.makeMiscGamePanel(C.accent3, 3)
...
;(function() -- 99 Nights in the Forest: STANDALONE + GUI INTEGRATION
local nnColor = Color3.fromRGB(80, 200, 100)
local nnContainer, nnContent, nnAddRow, _nnFolderApi = _sc.makeMiscGamePanel(nnColor, 4)
...
-- REMOTE EVENTS / FOLDER REFERENZEN (Asynchron abrufen um Ladeverzögerung zu vermeiden)
local RemoteEvents, remoteConsume, itemsFolder, characterFolder, tempStorage
task.spawn(function()
    RemoteEvents    = ReplicatedStorage:WaitForChild("RemoteEvents", 1)
    remoteConsume   = RemoteEvents and RemoteEvents:WaitForChild("RequestConsumeItem", 1)
    itemsFolder     = workspace:WaitForChild("Items", 1)
    characterFolder = workspace:WaitForChild("Characters", 1)
    tempStorage     = ReplicatedStorage:WaitForChild("TempStorage", 1)
end)
local bulkSources     = {itemsFolder, tempStorage}

-- TOOL / DAMAGE ID TABELLE
...
local TREE_ESP_ENABLED  = false
local ALWAYS_FEED_LOGS  = false
...
local function getAllToolContainers()
	local list = {LocalPlayer.Character}
	local invNames = {"Inventory","Backpack","PlayerInventory","Items","Tools","Hotbar"}
	for _, iName in ipairs(invNames) do
		local c = LocalPlayer:FindFirstChild(iName)
...
local function findToolAnywhere(toolName)
	for _, container in ipairs(getAllToolContainers()) do
		if container then
...
local function getAnyToolWithDamageID()
	...
	if char then
		for toolName, damageID in pairs(toolsDamageIDs) do
			if char:FindFirstChild(toolName) then return char:FindFirstChild(toolName), damageID end
		end
	end
	for toolName, damageID in pairs(toolsDamageIDs) do
		local tool = findToolAnywhere(toolName)
...
local function equipTool(tool)
	if not tool or equippedTool == tool or not RemoteEvents then return end
	pcall(function() RemoteEvents.EquipItemHandle:FireServer(tool) end)
	equippedTool = tool
end
		...
		if KILL_AURA_ENABLED then
			...
			if hrp and RemoteEvents then
				...
				if tool and damageID then
					equipTool(tool)
					local mobs = characterFolder and characterFolder:GetChildren() or {}
					for _, mob in ipairs(mobs) do
						if mob:IsA("Model") then
							local part = mob:FindFirstChildWhichIsA("BasePart")
							if part and (part.Position - hrp.Position).Magnitude <= KILL_AURA_RADIUS then
...
local function applyWalkSpeed(humanoid)
	if not humanoid then return end
	local speed = tonumber(_G.HackedWalkSpeed) or 16
	humanoid.WalkSpeed = speed
	humanoid.JumpPower = JUMP_POWER
	humanoid.Changed:Connect(function(property)
		if property == "WalkSpeed" and humanoid.WalkSpeed ~= tonumber(_G.HackedWalkSpeed) then
			humanoid.WalkSpeed = tonumber(_G.HackedWalkSpeed) or 16
		end
		if property == "JumpPower" and humanoid.JumpPower ~= JUMP_POWER then
			humanoid.JumpPower = JUMP_POWER
		end
	end)
end

local function hookCharacterStats()
...
local function createItemESP(model)
	if not model:IsA("Model") or not ITEM_ESP_ENABLED then return end
	if model:FindFirstChild("ItemESP_BB") then return end
	local part = model.PrimaryPart or model:FindFirstChildWhichIsA("BasePart")
	if not part then return end
	local bb = Instance.new("BillboardGui", model); bb.Name = "ItemESP_BB"
	bb.Size = UDim2.new(0, 100, 0, 30); bb.Adornee = part; bb.AlwaysOnTop = true; bb.StudsOffset = Vector3.new(0, 3, 0)
	local lbl = Instance.new("TextLabel", bb); lbl.Size = UDim2.new(1, 0, 1, 0); lbl.BackgroundTransparency = 1
	lbl.TextColor3 = Color3.new(1, 1, 1); lbl.TextStrokeTransparency = 0.4; lbl.Text = model.Name; lbl.TextSize = 14
end
...
task.spawn(function()
    while true do
        task.wait(3)
        if AUTO_EAT_ENABLED and RemoteEvents then
            local available = {}
            local items = itemsFolder and itemsFolder:GetChildren() or {}
            for _, item in ipairs(items) do
                if item.Name == "Cooked Steak" or item.Name == "Cooked Morsel" then table.insert(available, item) end
            end
...
-- [UI ROWS]
nnAddRow("Kill Aura", "Combat", nnColor, false, function(on) KILL_AURA_ENABLED = on end)
nnAddRow("Hack Speed", "Movement", nnColor, false, function(on) if on then hookCharacterStats() end end)
nnAddRow("Auto Eat", "Misc", nnColor, false, function(on) AUTO_EAT_ENABLED = on end)
nnAddRow("Auto Feed Logs", "Misc", nnColor, false, function(on) ALWAYS_FEED_LOGS = on end)
nnAddRow("Item ESP", "Visual", nnColor, false, function(on) 
    ITEM_ESP_ENABLED = on 
    local items = itemsFolder and itemsFolder:GetChildren() or {}
    if on then for _, m in ipairs(items) do createItemESP(m) end
    else for _, m in ipairs(items) do if m:FindFirstChild("ItemESP_BB") then m.ItemESP_BB:Destroy() end end end
end)
nnAddRow("Player ESP", "Visual", nnColor, false, function(on) PLAYER_ESP_ENABLED = on end)
nnAddRow("Tree HP ESP", "Visual", nnColor, false, function(on) TREE_ESP_ENABLED = on end)
nnAddRow("Axe Hitbox", "Combat", nnColor, false, function(on) HITBOX_ENABLED = on end)

_miscApis["99Nights"] = _nnFolderApi
pcall(function() _nnFolderApi.setActive(false) end)
end)() -- Ende 99 Nights IIFE
...
local shRow=Instance.new("Frame",_sc.sonstigePage)
...
_sc.sonstigePage.Size = UDim2.new(1, 0, 0, 228)
...
_sc.subPages = { Troll = trollPage, Movement = movePage, Visual = visualPage, Misc = _sc.sonstigePage, Combat = combatPage }
_sc.baseH = S_CARD_H + 62
_sc.activeCat = nil
_sc.catBtns = {}
_sc.switchSCat = function(id)
    ...
    if _sc.activeCat == id then
        ...
        twP(p, 0.18, {Size = UDim2.new(0, PANEL_W, 0, _sc.baseH)}, Enum.EasingStyle.Quart, Enum.EasingDirection.In)
        task.delay(0.2, function() pcall(function() p.ClipsDescendants = false end) end)
        c.Size = UDim2.new(1, 0, 0, _sc.baseH - 56)
...
p.Size = UDim2.new(0, PANEL_W, 0, _sc.baseH)
...
_TL_state.actions = {}
-- Forward references for actions are now part of _TL_state.actions
_SOH = {  
...
_TL_state.actions.stopSitOnHead = function()
    _SOH.active = false
    if _SOH.hoverVel then pcall(function() _SOH.hoverVel:Destroy() end); _SOH.hoverVel = nil end
    ...
    if _SOH.conn     then pcall(function() _SOH.conn:Disconnect() end);  _SOH.conn     = nil end
    _SOH.target = nil
    sohStopAnim()
    local myChar = LocalPlayer.Character
    local hum = myChar and myChar:FindFirstChildOfClass("Humanoid")
...
local function sohStopAnim()
    if _SOH.animTrack then pcall(function() _SOH.animTrack:Stop(0.1) end); _SOH.animTrack = nil end
    if _SOH.animConn  then pcall(function() _SOH.animConn:Disconnect() end); _SOH.animConn = nil end
    if _SOH.charConn  then pcall(function() _SOH.charConn:Disconnect() end); _SOH.charConn = nil end
end
...
pussySpreadActive=false, doggyActive=false, origGodState=false,
hugActive=false, hug2Active=false, qa74Active=false,
...
p.Size = UDim2.new(0, PANEL_W, 0, 276) -- Fit to internal cards (ends at 210 + 66 padding)
...
pcall(function() local r=LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart"); if r then r.Anchored=false end end)
...
if _AF.doggyActive      then stopDoggy();         _AF.doggyActive      = false end
...
local function execute1XAction(targetPlayer, animId, notifName, emoji, zOffset, speed)
    ...
    pcall(function() myRoot.Anchored = true end)
    ...
    if track then
        ...
        task.spawn(function()
            local t = 0
            while track.Length == 0 and t < 1 do task.wait(0.1); t = t + 0.1 end
            local waitTime = track.Length > 0 and track.Length or 1.5
...
elseif key == "doggy"     then startDoggy(target);     return true
...
elseif key == "headbutt" then task.spawn(function() execute1XAction(target, "81011129131522", "Headbutt", "💥", -1.3, 1.5) end); return true
elseif key == "1x_kiss"  then task.spawn(function() execute1XAction(target, "83995690642462", "Kiss", "💋", -1.5) end); return true
elseif key == "1x_slap"  then task.spawn(function() execute1XAction(target, "118273721275238", "Slap", "👋", -2.5) end); return true
elseif key == "1x_hug"   then task.spawn(function() execute1XAction(target, "90892690280238", "Hug", "🫂", -1.5) end); return true
elseif key == "jumpscare_vr" then task.spawn(function() execute1XAction(target, "114701433631329", "Jumpscare VR", "👻", -3.5) end); return true
end
return false
end,
}
...
pcall(sethiddenproperty, myRoot, "PhysicsRepRootPart", tgtRoot)
local tgtCF0 = tgtRoot.CFrame * CFrame.new(0, -0.5, -2.5) * _CF_ROT180Y
...
oscTime = oscTime + dt * LICKING_SPEED
        local tPelvis = tc:FindFirstChild("LowerTorso") or tc:FindFirstChild("Torso") or torso
        local tgtCF = tPelvis.CFrame * CFrame.new(0, -0.5, -2.5 - math.sin(oscTime)*0.4) * _CF_ROT180Y
...
pcall(function() myRoot:SetNetworkOwner(LocalPlayer) end)
pcall(sethiddenproperty, myRoot, "PhysicsRepRootPart", tgtRoot)
local tgtCF0 = tgtRoot.CFrame * CFrame.new(0, 1.7, -2.0) * _CF_ROT180Y
pcall(function() myRoot.CFrame = tgtCF0; myRoot.AssemblyLinearVelocity = _V3_ZERO end)
local bv = Instance.new("BodyVelocity")
bv.MaxForce = Vector3.new(0, 1e6, 0); bv.Velocity = _V3_ZERO
...
local tPelvis = tc:FindFirstChild("LowerTorso") or tc:FindFirstChild("Torso") or torso
local tgtCF = tPelvis.CFrame * CFrame.new(0, 1.7, -2.0 - math.sin(oscTime)*1.0) * _CF_ROT180Y
pcall(function() myR.CFrame = tgtCF; myR.AssemblyLinearVelocity = _V3_ZERO end)
local h2 = _myC and _myC:FindFirstChildOfClass("Humanoid")
if h2 and not h2.PlatformStand then h2.PlatformStand = true end
pcall(function() if h2:GetState()==Enum.HumanoidStateType.Seated then h2:SetStateEnabled(Enum.HumanoidStateType.Seated,false); h2:ChangeState(Enum.HumanoidStateType.Physics) end end)
...
local tPelvis = tc:FindFirstChild("LowerTorso") or tc:FindFirstChild("Torso") or torso
local tgtCF = tPelvis.CFrame * CFrame.new(0, -0.5, -3.1 - math.sin(oscTime)*0.5) * _CF_SUCK_ROT
pcall(function() myR.CFrame = tgtCF; myR.AssemblyLinearVelocity = _V3_ZERO end)
local h2 = _myC and _myC:FindFirstChildOfClass("Humanoid")
if h2 and not h2.PlatformStand then h2.PlatformStand = true end
pcall(function() if h2:GetState()==Enum.HumanoidStateType.Seated then h2:SetStateEnabled(Enum.HumanoidStateType.Seated,false); h2:ChangeState(Enum.HumanoidStateType.Physics) end end)
...
backshotsStartAnim()
pcall(function()
myRoot.CFrame = tHRP.CFrame * CFrame.new(0, -1.8, -2.0) * CFrame.Angles(math.rad(20) + 0.03, 0, 0)
pcall(function() myRoot.AssemblyLinearVelocity = _V3_ZERO end)
pcall(function() myRoot.AssemblyAngularVelocity = _V3_ZERO end)
end)
...
pcall(function()
local tPelvis = tc:FindFirstChild("LowerTorso") or tc:FindFirstChild("Torso") or tHRP2
local offset = -2.0 - math.sin(oscTime) * 1.5
myR.CFrame = tPelvis.CFrame
* CFrame.new(0, -1.8, offset)
...
do local function _TLact_Doggy() do
local doggyConn      = nil
local doggyTarget    = nil
local doggyBodyPos   = nil
...
local function doggyStopAnim()
    if doggyAnimTrack then doggyAnimTrack:Stop(); doggyAnimTrack = nil end
    if doggyAnimConn  then doggyAnimConn:Disconnect();  doggyAnimConn  = nil end
    if doggyCharConn  then doggyCharConn:Disconnect();  doggyCharConn  = nil end
end
...
local function doggyStartAnim()
    ...
    doggyCharConn = LocalPlayer.CharacterAdded:Connect(function(char)
        if _AF.doggyActive then task.wait(0.5); task.spawn(function() doggyPlayAnim(char) end) end
    end)
end

stopDoggy = function()
    _AF.doggyActive = false; doggyTarget = nil
    if doggyConn    then doggyConn:Disconnect();    doggyConn    = nil end
    if doggyBodyPos then pcall(function() doggyBodyPos:Destroy() end); doggyBodyPos = nil end
    doggyStopAnim()
    setFreeze(false)
    local hum = getHumanoid(); if hum and not flyActive then hum.PlatformStand = false; pcall(function() hum:SetStateEnabled(Enum.HumanoidStateType.Seated, true) end) end
    if not flyActive then pcall(function()
...
startDoggy = function(targetPlayer)
    ...
    local myChar     = LocalPlayer.Character
    local targetChar = targetPlayer and targetPlayer.Character
    if not myChar or not targetChar then sendNotif("Doggy", "No character!", 2); return end
    local myRoot = myChar:FindFirstChild("HumanoidRootPart")
    local tHRP   = targetChar:FindFirstChild("HumanoidRootPart")
    if not myRoot or not tHRP then sendNotif("Doggy", "Missing parts!", 2); return end
    local hum = getHumanoid(); if hum then hum.PlatformStand = true; pcall(function() hum:SetStateEnabled(Enum.HumanoidStateType.Seated, false) end) end
    pcall(function() myRoot:SetNetworkOwner(LocalPlayer) end)
    pcall(sethiddenproperty, myRoot, "PhysicsRepRootPart", tHRP)
    local bv = Instance.new("BodyVelocity")
    bv.MaxForce = Vector3.new(0, 1e6, 0)
    bv.Velocity = Vector3.zero
    bv.Parent = myRoot; doggyBodyPos = bv
    local oscTime = 0
    local DG_SPEED = 10.0
    ...
    doggyStartAnim()
    pcall(function()
        local tPelvis = targetChar:FindFirstChild("LowerTorso") or targetChar:FindFirstChild("Torso") or tHRP
        myRoot.CFrame = tPelvis.CFrame * CFrame.new(0, 0.5, -0.8)
        pcall(function() myRoot.AssemblyLinearVelocity = _V3_ZERO end)
    end)
    doggyConn = _RSConnect(function(dt)
        ...
        local myR   = _lpc and _lpc:FindFirstChild("HumanoidRootPart")
        if not tHRP2 or not myR then return end
        if not doggyBodyPos or doggyBodyPos.Parent ~= myR then
            if doggyBodyPos then pcall(function() doggyBodyPos:Destroy() end) end
            local bv2 = Instance.new("BodyVelocity")
            bv2.MaxForce = Vector3.new(0, 1e6, 0)
            bv2.Velocity = Vector3.zero; bv2.Parent = myR; doggyBodyPos = bv2
        end
        pcall(function()
            local tPelvis2 = tc:FindFirstChild("LowerTorso") or tc:FindFirstChild("Torso") or tHRP2
            myR.CFrame = tPelvis2.CFrame * CFrame.new(0, 0.2, -0.8) * CFrame.Angles(math.rad(-90), 0, 0)
            pcall(function() myR.AssemblyLinearVelocity = _V3_ZERO end)
        end)
    end)
end
end end _TLact_Doggy() end
...
pcall(function()
        local tPelvis = tc:FindFirstChild("LowerTorso") or tc:FindFirstChild("Torso") or tHRP2
        local offset = -2.0 - math.sin(oscTime) * 1.5
        myR.CFrame = tPelvis.CFrame * CFrame.new(0, 0, offset)
        pcall(function() myR.AssemblyLinearVelocity = _V3_ZERO end)
end)
local h2 = _myC and _myC:FindFirstChildOfClass("Humanoid")
if h2 and not h2.PlatformStand then h2.PlatformStand = true end
...
hrp.Anchored = false
hrp.AssemblyLinearVelocity = _V3_ZERO
hrp.AssemblyAngularVelocity = _V3_ZERO
if orbitBodyPos then pcall(function() orbitBodyPos:Destroy() end); orbitBodyPos = nil end
-- Zweite Sicherheit: Alle Body-Objekte im HRP löschen
pcall(function()
for _, o in ipairs(hrp:GetChildren()) do
if o:IsA("BodyVelocity") or o:IsA("BodyPosition") or o:IsA("BodyGyro") then o:Destroy() end
end
end)
end
...
-- Delayed cleanup pass (wie bei Kiss/SuckIt)
...
if r2 then
pcall(function() sethiddenproperty(r2, "PhysicsRepRootPart", nil) end)
r2.AssemblyLinearVelocity = _V3_ZERO
end
if h2 then
...
local offset = _V3new(_cp * orbitRadius, 2.5 + _msin(phase*3)*1.5, _sp * orbitRadius)
...
if bbAnimTrack8_ then
pcall(function() bbAnimTrack8_:AdjustSpeed(1); bbAnimTrack8_:Stop() end)
bbAnimTrack8_ = nil
end
...
local function bbPlayAnim1(char)
    if not char or not _AF.bbActive then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local track = _AF_getReliableActionTrack(hum, PIGGYBACK_ANIM_ID, "BBPiggybackAnim")
    if not track then return end
    bbAnimTrack_ = track
    if bbAnimConn_ then bbAnimConn_:Disconnect() end
    bbAnimConn_ = track.Stopped:Connect(function()
        if _AF.bbActive and bbMode_ == "bb_piggyback" then task.wait(0.1); bbPlayAnim1(char) end
    end)
end
local function bbPlayAnim2(char)
    if not char or not _AF.bbActive then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local track = _AF_getReliableActionTrack(hum, PIGGYBACK2_ANIM_ID, "BBPiggyback2Anim")
    if not track then return end
    bbAnimTrack2_ = track
    if bbAnimConn2_ then bbAnimConn2_:Disconnect() end
    bbAnimConn2_ = track.Stopped:Connect(function()
        if _AF.bbActive and bbMode_ == "bb_piggyback2" then task.wait(0.1); bbPlayAnim2(char) end
    end)
end
local function bbPlayAnim3(char)
    if not char or not _AF.bbActive then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local track = _AF_getReliableActionTrack(hum, BB_BACKSHOTS_ANIM_ID, "BBBackshotsAnim")
    if not track then return end
    bbAnimTrack3_ = track
    if bbAnimConn3_ then bbAnimConn3_:Disconnect() end
    bbAnimConn3_ = track.Stopped:Connect(function()
        if _AF.bbActive and bbMode_ == "bb_attach" then task.wait(0.1); bbPlayAnim3(char) end
    end)
end
local function bbPlayAnim4(char)
    if not char or not _AF.bbActive then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local track = _AF_getReliableActionTrack(hum, BB_CARRY_ANIM_ID, "BBCarryAnim")
    if not track then return end
    bbAnimTrack4_ = track
    if bbAnimConn4_ then bbAnimConn4_:Disconnect() end
    bbAnimConn4_ = track.Stopped:Connect(function()
        if _AF.bbActive and bbMode_ == "bb_carry" then task.wait(0.1); bbPlayAnim4(char) end
    end)
end
...
local function bbPlayAnim5(char)
    if not char or not _AF.bbActive then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local track = _AF_getReliableActionTrack(hum, BB_BANGV2_ANIM_ID, "BBBangV2Anim")
    if not track then return end
    pcall(function() track:AdjustSpeed(2) end)
    bbAnimTrack5_ = track
    if bbAnimConn5_ then bbAnimConn5_:Disconnect() end
    bbAnimConn5_ = track.Stopped:Connect(function()
        if _AF.bbActive and bbMode_ == "bb_bangv2" then task.wait(0.1); bbPlayAnim5(char) end
    end)
end
local function bbPlayAnim6(char)
    if not char or not _AF.bbActive then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local track = _AF_getReliableActionTrack(hum, BB_CARRY2_ANIM_ID, "BBCarry2Anim")
    if not track then return end
    bbAnimTrack6_ = track
    if bbAnimConn6_ then bbAnimConn6_:Disconnect() end
    bbAnimConn6_ = track.Stopped:Connect(function()
        if _AF.bbActive and bbMode_ == "bb_carry2" then task.wait(0.1); bbPlayAnim6(char) end
    end)
end
local function bbPlayAnim7(char)
    if not char or not _AF.bbActive then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local track = _AF_getReliableActionTrack(hum, BB_HUG_ANIM_ID, "BBHugAnim")
    if not track then return end
    bbAnimTrack7_ = track
    if bbAnimConn7_ then bbAnimConn7_:Disconnect() end
    bbAnimConn7_ = track.Stopped:Connect(function()
        if _AF.bbActive and bbMode_ == "bb_hug" then task.wait(0.1); bbPlayAnim7(char) end
    end)
end
local function bbPlayAnim8(char)
    if not char or not _AF.bbActive then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local track = _AF_getReliableActionTrack(hum, BB_LICKING_ANIM_ID, "BBLickingAnim")
    if not track then return end
    bbAnimTrack8_ = track
    if bbAnimConn8_ then bbAnimConn8_:Disconnect() end
    bbAnimConn8_ = track.Stopped:Connect(function()
        if _AF.bbActive and bbMode_ == "bb_licking" then task.wait(0.1); bbPlayAnim8(char) end
    end)
end
local function bbPlayAnim9(char)
    if not char or not _AF.bbActive then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local track = _AF_getReliableActionTrack(hum, BB_HUG2_ANIM_ID, "BBHug2Anim")
    if not track then return end
    bbAnimTrack9_ = track
    if bbAnimConn9_ then bbAnimConn9_:Disconnect() end
    bbAnimConn9_ = track.Stopped:Connect(function()
        if _AF.bbActive and bbMode_ == "bb_hug2" then task.wait(0.1); bbPlayAnim9(char) end
    end)
end
local function bbPlayAnim10(char)
    if not char or not _AF.bbActive then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local track = _AF_getReliableActionTrack(hum, BB_LAYFUCK_ANIM_ID, "BBLayFuckAnim")
    if not track then return end
    bbAnimTrack10_ = track
    if bbAnimConn10_ then bbAnimConn10_:Disconnect() end
    bbAnimConn10_ = track.Stopped:Connect(function()
        if _AF.bbActive and bbMode_ == "bb_layfuck" then task.wait(0.1); bbPlayAnim10(char) end
    end)
end
stopBB = function()
...
mHRP.CFrame = torso.CFrame * CFrame.new(0, 0, offset)
mHRP.AssemblyLinearVelocity = _V3_ZERO
_guardHum()
...
local function makePillBtn(parent, xScale, xOff, w, label, accentC)
    ...
    f.BorderSizePixel   = 0
    corner(f, 6)   -- eckig abgerundet (statt vollrund)
...
local function rebuildList()
    ...
    if visIdx == 0 and _plFilterText ~= "" then
        noResultsLbl.Text = "Kein Spieler gefunden für: \"" .. _plFilterText .. "\""
...
local _, invisAnimSet = subRow(genPage, 162, "Invisible Animation", "ghost effect", C.accent, _invisAnimEnabled, function(on)
    _invisAnimEnabled = on
end)
_G.settingToggleSetters = settingToggleSetters
genPage.Size = UDim2.new(1, 0, 0, 162 + 54 + 8)
end) -- /_ok_genPage
local kbPage
...
{ "Toggle Noclip",    nil,                       function(input)
-- Ignore Noclip keybind if we are flying and pressing Q (to prevent overlap with Fly Boost)
if input and typeof(input) == "Instance" and input:IsA("InputObject") then
    if flyActive and input.KeyCode == Enum.KeyCode.Q then return end
end
...
local function _makeChipRow(sectionLabel, items, onSelect, getActive)
    ...
    for i,item in ipairs(items) do
        ...
        btn.MouseButton1Click:Connect(activate)
        btn.InputBegan:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.Touch then activate() end end)
        btn.MouseEnter:Connect(function() _sc._playHoverSound(); if getActive()~=captId then twP(chip,0.1,{BackgroundColor3=C.bg3}) end end)
...
do
    ...
    for row = 0, 3 do
        ...
        for col = 0, 4 do
            ...
            fb.MouseEnter:Connect(function() _sc._playHoverSound(); f.BackgroundTransparency = 0.15 end)
...
function getNearestPlayer()
    ...
    local myPos = (_G._TLInvisActive and _G._TLInvisSavedCF) and _G._TLInvisSavedCF.Position or myRoot.Position
...
-- -- FPS + Ping Widget Loop (Rolling 60-sample average + color coding) --
local _fwSamples  = {}
local _FW_WINDOW  = 60  -- rolling window size
local _fwPingAcc  = 0
local _fwPingIval = 0.5 -- ping read interval in seconds
local _fwLastPing = 0

local _fwStatsService; pcall(function() _fwStatsService = game:GetService("Stats") end)
local _fwPingItem
pcall(function()
    if _fwStatsService then
        _fwPingItem = _fwStatsService.Network.ServerStatsItem["Data Ping"]
    end
end)

_tlTrackConn(_SvcRS.Heartbeat:Connect(function(dt)
    if not _tlAlive() then return end
    -- Rolling FPS accumulation
    table.insert(_fwSamples, 1 / math.max(dt, 0.001))
    if #_fwSamples > _FW_WINDOW then table.remove(_fwSamples, 1) end
    -- Throttled display update (every 0.5s)
    ...
    if _fwPingAcc >= _fwPingIval then
        ...
        -- Read ping via official Roblox Stats API
        local pingOk, pVal = pcall(function() return _fwPingItem:GetValue() end)
        if pingOk and pVal then _fwLastPing = _mfloor(pVal) end
        -- FPS color: ≥60 green, ≥30 yellow, <30 red
        local fpsCol
        if fps >= 60 then fpsCol = Color3.fromRGB(100, 215, 130)
        elseif fps >= 30 then fpsCol = Color3.fromRGB(230, 195, 70)
        else fpsCol = Color3.fromRGB(235, 80, 80) end
        -- Ping color: ≤80 green, ≤150 yellow, >150 red
        local pingCol
        if _fwLastPing <= 80 then pingCol = Color3.fromRGB(100, 215, 130)
        elseif _fwLastPing <= 150 then pingCol = Color3.fromRGB(230, 195, 70)
        else pingCol = Color3.fromRGB(235, 80, 80) end
        if fwVal and fwVal.Parent then
            fwVal.Text       = tostring(fps)
            fwVal.TextColor3 = fpsCol
            if liveDot then liveDot.BackgroundColor3 = fpsCol end
        end
...
;(function() -- Scope-Isolation for QuickAction (IIFE)
local _q = {} -- Shared QuickAction table to save registers
_q.statusDot = nil
_q.statusTxt = nil
task.spawn(function()
...
_qb.g       = _TL_refs
_TL_refs._TL_qb = _qb
_qb.env   = (getgenv and getgenv()) or {}
_qb.ScreenGui  = _qb.g._TL_ScreenGui  or _qb.env._TL_ScreenGui
_qb.tw         = _qb.g._TL_tw         or _qb.env._TL_tw
_qb.tlHitbox   = _qb.g._TL_tlHitbox   or _qb.env._TL_tlHitbox
_qb.tlLbl      = _qb.g._TL_tlLbl      or _qb.env._TL_tlLbl
_qb.tlArrow    = _qb.g._TL_tlArrow    or _qb.env._TL_tlArrow
_qb.tlArrowBig = _qb.g._TL_tlArrowBig or _qb.env._TL_tlArrowBig
_qb.FW_W       = _qb.g._TL_FW_W       or _qb.env._TL_FW_W       or 230
_qb.FW_H       = _qb.g._TL_FW_H       or _qb.env._TL_FW_H       or 34
_qb.FW_X_OFFSET= _qb.g._TL_FW_X_OFFSET or _qb.env._TL_FW_X_OFFSET or -5
_qb.VL_ICON_W  = _qb.g._TL_VL_ICON_W  or _qb.env._TL_VL_ICON_W  or 58
_qb.sendNotif  = _qb.g._TL_sendNotif  or _qb.env._TL_sendNotif
_qb.getRootPart= _qb.g._TL_getRootPart or _qb.env._TL_getRootPart
_qb.safeStand  = _qb.g._TL_safeStand  or _qb.env._TL_safeStand
_qb.AF         = _qb.g._TL_AF         or _qb.env._TL_AF
_qb.SOH        = _qb.g._TL_SOH        or _qb.env._TL_SOH
_qb.act_stopFollow  = _qb.g._TL_act_stopFollow  or _qb.env._TL_act_stopFollow
_qb.stopBB           = _qb.g._TL_stopBB          or _qb.env._TL_stopBB
_qb.startBB          = _qb.g._TL_startBB         or _qb.env._TL_startBB
_qb.stopGhost        = _qb.g._TL_stopGhost       or _qb.env._TL_stopGhost
_qb.stopSitOnHead    = _qb.g._TL_stopSitOnHead   or _qb.env._TL_stopSitOnHead
_qb.stopPiggyback    = _qb.g._TL_stopPiggyback   or _qb.env._TL_stopPiggyback
_qb.stopPiggyback2   = _qb.g._TL_stopPiggyback2  or _qb.env._TL_stopPiggyback2
_qb.stopKiss         = _qb.g._TL_stopKiss        or _qb.env._TL_stopKiss
_qb.stopBackpack     = _qb.g._TL_stopBackpack    or _qb.env._TL_stopBackpack
_qb.stopOrbit        = _qb.g._TL_stopOrbit       or _qb.env._TL_stopOrbit
_qb.stopUpsideDown   = _qb.g._TL_stopUpsideDown  or _qb.env._TL_stopUpsideDown
_qb.stopCrossUD      = _qb.g._TL_stopCrossUD     or _qb.env._TL_stopCrossUD
_qb.stopFriend       = _qb.g._TL_stopFriend      or _qb.env._TL_stopFriend
_qb.stopSpinning     = _qb.g._TL_stopSpinning    or _qb.env._TL_stopSpinning
_qb.stopLicking      = _qb.g._TL_stopLicking      or _qb.env._TL_stopLicking
_qb.stopSucking      = _qb.g._TL_stopSucking      or _qb.env._TL_stopSucking
_qb.stopSuckIt       = _qb.g._TL_stopSuckIt       or _qb.env._TL_stopSuckIt
_qb.stopBackshots    = _qb.g._TL_stopBackshots    or _qb.env._TL_stopBackshots
_qb.stopDoggy        = _qb.g._TL_stopDoggy        or _qb.env._TL_stopDoggy
_qb.stopLayFuck      = _qb.g._TL_stopLayFuck      or _qb.env._TL_stopLayFuck
_qb.stopFacefuck     = _qb.g._TL_stopFacefuck     or _qb.env._TL_stopFacefuck
_qb.stopPussySpread  = _qb.g._TL_stopPussySpread  or _qb.env._TL_stopPussySpread
_qb.stopHug          = _qb.g._TL_stopHug          or _qb.env._TL_stopHug
_qb.stopHug2         = _qb.g._TL_stopHug2         or _qb.env._TL_stopHug2
_qb.stopCarry        = _qb.g._TL_stopCarry        or _qb.env._TL_stopCarry
_qb.stopShoulderSit  = _qb.g._TL_stopShoulderSit  or _qb.env._TL_stopShoulderSit
...
_qb.QA_COLS       = 3
_qb.QA_GAP        = 5
_qb.QA_CW         = math.floor((_qb.QA_W - _qb.QA_PAD*2 - _qb.QA_GAP*2) / _qb.QA_COLS)
_qb.QA_CH         = 68
_qb.HDR_H         = 38
_qb.SEC_H         = 20
_qb.FOOT_H        = 30
...
{
label="1X Action", col=Color3.fromRGB(180, 80, 255),
actions={
    { key="headbutt", label="Headbutt", imageId="rbxassetid://102559848050770" },
...
_qb.qaCardRefs     = {}
_qb.qaNoSitConn   = nil  -- verhindert Hinsetzen während QA-Action läuft
_qb.qaGlobalLock  = false  -- verhindert Race beim schnellen Doppelklick verschiedener Karten

_qb.qaStopNoSit = function()
...
_qb.qaStartNoSit = function()
	...
	local _nsChar = nil
	local _nsHum  = nil
	_qb.qaNoSitConn = RunService.Heartbeat:Connect(function()
		local char = LocalPlayer.Character
		if not char then return end
...
_qb.stopQAAction = function()
_qb.qaStopNoSit()
if _G.TLActionsStop then pcall(_G.TLActionsStop)
elseif _G.TLActions  then pcall(function() _G.TLActions.stopAll() end) end
pcall(function()
local _af = _qb.AF or {}
if _qb.act_following     then _qb.act_stopFollow(); _qb.act_following = false end
if _qb.SOH and _qb.SOH.active then _qb.stopSitOnHead(); _qb.SOH.active = false end
if _qb.ppActive          then _qb.stopPiggyback();  _qb.ppActive          = false end
if _af.pp2Active         then _qb.stopPiggyback2(); _af.pp2Active         = false end
if _af.kissActive        then _qb.stopKiss();       _af.kissActive        = false end
if _af.backpackActive    then _qb.stopBackpack();   _af.backpackActive    = false end
if _af.orbitActive       then _qb.stopOrbit();      _af.orbitActive       = false end
if _af.upsideDownActive  then _qb.stopUpsideDown(); _af.upsideDownActive  = false end
if _af.crossUDActive     then _qb.stopCrossUD();    _af.crossUDActive     = false end
if _af.friendActive      then _qb.stopFriend();     _af.friendActive      = false end
if _af.spinningActive    then _qb.stopSpinning();   _af.spinningActive    = false end
if _af.lickingActive     then _qb.stopLicking();    _af.lickingActive     = false end
if _af.suckingActive     then _qb.stopSucking();    _af.suckingActive     = false end
if _af.suckItActive      then _qb.stopSuckIt();     _af.suckItActive      = false end
if _af.backshotsActive   then _qb.stopBackshots();  _af.backshotsActive   = false end
if _af.doggyActive       then _qb.stopDoggy();      _af.doggyActive       = false end
if _af.layFuckActive     then _qb.stopLayFuck();    _af.layFuckActive     = false end
if _af.facefuckActive    then _qb.stopFacefuck();   _af.facefuckActive    = false end
if _af.pussySpreadActive then _qb.stopPussySpread(); _af.pussySpreadActive = false end
if _af.hugActive         then _qb.stopHug();        _af.hugActive         = false end
if _af.hug2Active        then _qb.stopHug2();       _af.hug2Active        = false end
if _af.carryActive       then _qb.stopCarry();      _af.carryActive       = false end
if _af.shoulderSitActive then _qb.stopShoulderSit(); _af.shoulderSitActive = false end
if _af.bbActive          then _qb.stopBB();         _af.bbActive          = false end
if _af.ghostActive       then _qb.stopGhost();      _af.ghostActive       = false end
if _af.qa74Active        then pcall(stopQA74);      _af.qa74Active        = false end
pcall(_qb.safeStand)
end)
_qb.qaActiveKey = nil; _qb.qaActiveTarget = nil
end
_qb.activateQAAction = function(key)
    local target = getNearestPlayer()
    if not target then 
        _qb.sendNotif("QuickActions", "Kein Spieler in der Nähe!", 2)
        return false 
    end
    ...
    task.spawn(function()
        ...
        local ok, err = xpcall(function()
            if _capturedKey:sub(1,3) == "bb_" then 
                if type(_qb.startBB) == "function" then
                    _qb.startBB(_capturedTarget, _capturedKey)
                else
                    warn("[QA] Error: _qb.startBB is not a function! (Type: " .. type(_qb.startBB) .. ")")
                end
            elseif _G.TLActions then 
                if type(_G.TLActions) == "table" and type(_G.TLActions.start) == "function" then
                    ...
                    warn("[QA] Error: _G.TLActions.start is not a function! (Type: " .. type(_G.TLActions.start) .. ")")
                end
            end
        end, function(e)
...
_qb.mkF = function(parent, sz, pos, col, alpha, r)
...
_qb.mkTxt = function(parent, sz, pos, text, font, tsz, col, xAlign)
...
_qb.mkStroke = function(parent, thick, col, alpha)
...
_qb.qaBar = _qb.mkF(_qb.ScreenGui,
...
_qb._qaBarStroke = _qb.mkStroke(_qb.qaBar, 1, P.panelBrd, 0.7)
...
local mkF = _qb.mkF
local mkTxt = _qb.mkTxt
local mkStroke = _qb.mkStroke
local HDR_H = _qb.HDR_H
_qb.hdr = _qb.mkF(_qb.qaBar, UDim2.new(1,0,0,HDR_H),
...
icoLbl.ZIndex                 = 12
_qb.titleLbl = _qb.mkTxt(hdr, UDim2.new(0,125,0,18), UDim2.new(0,32,0.5,-9),
...
_qb.tgtBadge = mkF(hdr, UDim2.new(0,90,0,18), UDim2.new(1,-98,0.5,-9), P.tgtBg, 0.1, 20)
local tgtBadge = _qb.tgtBadge
_qb._tgtBadgeStroke = _qb.mkStroke(tgtBadge, 1.5, C.accent2, 0); tgtBadge.ZIndex=11
local tgtDot = mkF(tgtBadge, UDim2.new(0,5,0,5), UDim2.new(0,6,0.5,-2), P.tgtDot, 0, 99)
tgtDot.ZIndex=13
_qb.tgtNameLbl = _qb.mkTxt(tgtBadge, UDim2.new(1,-16,1,0), UDim2.new(0,14,0,0),
...
_tdTw = _qb.tw(tgtDot,0.8,{BackgroundTransparency=0.6}); _tdTw:Play()
task.wait(1.7)
if _tdTw then pcall(_tdTw.Cancel, _tdTw) end
_tdTw = _qb.tw(tgtDot,0.8,{BackgroundTransparency=0}); _tdTw:Play()
task.wait(1.7)
else
task.wait(0.5)
end
...
_panelColorHooks[#_panelColorHooks+1] = function(_newT)
    ...
    pcall(function() if _qb._tgtBadgeStroke then _qb._tgtBadgeStroke.Color = C.accent2  end end)
...
for _, r in ipairs(_qb.qaCardRefs) do pcall(function()
...
local xPos  = col_i * (_qb.QA_CW + _qb.QA_GAP)
...
bg.ZIndex=11
local stroke = _qb.mkStroke(bg, 1, P.cardBrd, 0.5)
...
btn.MouseEnter:Connect(function()
if _isMobile then return end
            pcall(function() if type(_sc._playHoverSound) == "function" then _sc._playHoverSound() end end)
...
local _cardLock = false   -- per-Karte-Lock: blockiert verzögerten zweiten Touch-Event
        local function qaCardActivate()
            local ok, err = xpcall(function()
                ...
                if wasActive then
                    _qb.stopQAAction()
                    if _q.statusTxt then _q.statusTxt.Text="Stopped"; _q.statusTxt.TextColor3=P.tgtTxt end
                    ...
                    if ok ~= false then
                        ...
                        task.spawn(function()
                            task.wait(0.2); pcall(function()
                                ...
                                if _q.statusTxt then
                                    _q.statusTxt.Text=act.label..(tgt and(" → "..tgt.Name)or"")
                        ...
                        if _q.statusTxt then _q.statusTxt.Text="◈ Kein Ziel"; _q.statusTxt.TextColor3=P.stopTxt end
                        if _q.statusDot then _q.statusDot.BackgroundColor3=P.stopTxt end
                    end
                end
            end, function(e)
                warn("[QA] Error in qaCardActivate for " .. tostring(act.key) .. ": " .. tostring(e))
                warn(debug.traceback())
            end)
        end
...
local function qaSuckPause()
    if _qb.qaActiveKey=="sucking" and _AF and _AF.suckingActive then
...
_sdTw = _qb.tw(_q.statusDot,0.7,{BackgroundTransparency=0.6}); _sdTw:Play()
task.wait(1.5)
if _sdTw then pcall(_sdTw.Cancel, _sdTw) end
_sdTw = _qb.tw(_q.statusDot,0.7,{BackgroundTransparency=0}); _sdTw:Play()
task.wait(1.5)
else
task.wait(0.5)
end
...
if _qb.qaBarTween then pcall(function() _qb.qaBarTween:Cancel() end); _qb.qaBarTween=nil end
closeBar()
local np = getNearestPlayer()
...
_qb.qaBarTween=_qb.tw(_qb.qaBar,.28,{Size=UDim2.new(0,_qb.QA_W,0,FULL_H)},
Enum.EasingStyle.Quart,Enum.EasingDirection.Out)
_qb.qaBarTween:Play()
_qb.tlArrow.Text                 = "»"
end
...
if _qb.qaBarTween then pcall(function() _qb.qaBarTween:Cancel() end); _qb.qaBarTween=nil end
_qb.qaBarOpen=false; _qb.tlArrow.Text                 = "»"
_qb.qaBarTween=_qb.tw(_qb.qaBar,.2,{Size=UDim2.new(0,_qb.QA_W,0,0)},
Enum.EasingStyle.Quart,Enum.EasingDirection.In)
_qb.qaBarTween:Play()
...
local _conns = {
_TL_state.conns.flyConn, _TL_state.conns.noclipConn, _TL_state.conns._espRadConn, _TL_state.conns.gbConn,
...
_AF and _AF.udConn,
_TL_state.conns.friendConn, _TL_state.conns.spinConn, _TL_state.conns.ppConn, _TL_state.conns.pp2Conn,
_TL_state.conns.kissConn, _TL_state.conns.bpConn, _TL_state.conns.lickingConn, _TL_state.conns.suckingConn,
_TL_state.conns.facefuckConn, _TL_state.conns.backshotsConn, _TL_state.conns.psConn, _TL_state.conns.hugConn,
_TL_state.conns.carryConn, _TL_state.conns.ssConn, _TL_state.conns.qa74Conn, _TL_state.conns.orbitConn,
_TL_state.conns.ghostConn, _TL_state.conns.bbConn, _TL_state.conns.bbRespConn, _TL_state.conns.bbRemConn,
_TL_state.conns.bbAnimConn_, _TL_state.conns.bbAnimConn2_, _TL_state.conns.bbAnimConn3_,
_TL_state.conns.bbAnimConn4_, _TL_state.conns.bbAnimConn5_, _TL_state.conns.bbAnimConn6_, _TL_state.conns.bbAnimConn7_,
_TL_state.conns.bbAnimConn8_, _TL_state.conns.bbAnimConn9_, _TL_state.conns.bbAnimConn10_, _TL_state.conns.bbHealthConn_,
...
_TL_state.conns.ppCharConn, _TL_state.conns.pp2CharConn, _TL_state.conns.kissCharConn, _TL_state.conns.bpCharConn,
...
_TL_state.conns.psCharConn, _TL_state.conns.hugCharConn, _TL_state.conns.hug2CharConn, _TL_state.conns.carryCharConn,
_TL_state.conns.ssCharConn, _TL_state.conns.qa74CharConn, _TL_state.conns.avCharConn, _TL_state.conns._ui_charConn_,
...
_TL_state.conns._ctHoverConn, _qb_ref.qaNoSitConn, _TL_state.conns._dhAimConn,
...
pcall(function()
local targets = {SmartBarGui=true, MatrixTrackerGUI=true, TL_ESP_Highlight=true}
local lp = _SvcPlr.LocalPlayer
local pg = lp:FindFirstChild("PlayerGui")
if pg then
for _, g in ipairs(pg:GetChildren()) do
if targets[g.Name] then pcall(function() g:Destroy() end) end
end
end
...
_TL_refs.TLMenuCleanup = _TL_refs.TLMenuCleanup
pcall(function()
local env = getgenv and getgenv() or _G
