...
do -- Inner scope for TL Fly
    ...
    StatusLabel.Size = UDim2.new(1, 0, 0, 12); StatusLabel.Position = UDim2.new(0, 0, 0, 17); StatusLabel.BackgroundTransparency = 1; StatusLabel.Text = "GLIDE"; StatusLabel.TextColor3 = C.sub
    ...
    local MinusBtn = createButton("−", function() if speedIndex > 1 then speedIndex -= 1; hasBoosted = true; updateFlyPanel() end end)
    MinusBtn.Size = UDim2.new(0, 26, 1, 0); MinusBtn.LayoutOrder = 2; MinusBtn.Parent = SpeedGroup
    local PlusBtn = createButton("+", function() if speedIndex < #speedLevels then speedIndex += 1; hasBoosted = true; updateFlyPanel() end end)
    ...
    for i, set in ipairs(animSets) do
        ...
        corner(row, 7)
        local rs2 = _makeDummyStroke(row)
        rs2.Color = Color3.fromRGB(80, 100, 120); rs2.Thickness = 0.8; rs2.Transparency = 0.7
...
local function makeSliderRow(yPos, label, sublabel, col, vMin, vMax, vDef, onToggle, onReset, onSlide)
    ...
    task.spawn(function()
        while _u.card and _u.card.Parent do
            local dt = task.wait()
            if not _u.card.Visible then continue end
            -- Lerp
...
local function bbPlayAnim4(char)
    if not char or not _AF.bbActive then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local track = _AF_getReliableActionTrack(hum, CARRY_ANIM_ID, "BBCarryAnim")
    if not track then return end
    bbAnimTrack4_ = track
...
local function bbPlayAnim6(char)
    if not char or not _AF.bbActive then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local track = _AF_getReliableActionTrack(hum, CARRY_ANIM_ID, "BBCarry2Anim")
    if not track then return end
    bbAnimTrack6_ = track
...
local function bbPlayAnim7(char)
    if not char or not _AF.bbActive then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local track = _AF_getReliableActionTrack(hum, HUG_ANIM_ID, "BBHugAnim")
    if not track then return end
    bbAnimTrack7_ = track
...
local function bbPlayAnim8(char)
    if not char or not _AF.bbActive then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local track = _AF_getReliableActionTrack(hum, LICKING_ANIM_ID, "BBLickingAnim")
    if not track then return end
    bbAnimTrack8_ = track
...
local function bbPlayAnim9(char)
    if not char or not _AF.bbActive then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local track = _AF_getReliableActionTrack(hum, HUG2_ANIM_ID, "BBHug2Anim")
    if not track then return end
    bbAnimTrack9_ = track
...
local function bbPlayAnim10(char)
    if not char or not _AF.bbActive then return end
    local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end
    local track = _AF_getReliableActionTrack(hum, LAYFUCK_ANIM_ID, "BBLayFuckAnim")
    if not track then return end
